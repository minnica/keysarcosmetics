...
- [ ] **RV5-P1 · Desarrollo, post-MVP:** materializar las revisiones del diálogo aprobado sobre tickets, pagos, inventario, membresías, Agenda y reportes mediante los motores y compensaciones existentes. Conservar el original y registrar actor, versión y diferencias; usar transacción o recuperación durable cuando intervengan efectos externos. Cierre: corrección visible tras recarga, importes/existencias/saldos conciliados y reintento/doble envío sin duplicados. Hasta entonces la revisión sólo se anuncia como registrada.
  ...
  - **Checkpoint parcial 5, 2026-09-16:** las revisiones ya admiten paquetes y cortesías cuando el diálogo conserva exactamente orden, identidad, cantidad y precio de sus líneas. El servidor reconstruye el `packageId` desde el ticket, bloquea `PosPackage`/`PosCourtesy`, valida su relación canónica con líneas `SALE`/`GIFT` y citas, excluye regalos de cotización e inventario y registra `packagePreservations`/`courtesyPreservations` antes/después y en efectos. Retirar, agregar, reordenar o sustituir composición responde `409` antes del token; un paquete retirado o fuera de vigencia tampoco se reinterpreta. PostgreSQL 16 reconstruyó 46 migraciones y pasó 10/10 casos POS más 3/3 POS/Scheduler: rechazo con token intacto, revisión/replay único de paquete, cortesía+cita byte-a-byte sin mutación ni duplicado y proyecciones financieras conciliadas. Schemas, tipos, lint, 136 unitarias, builds, formato, manifiesto y diff pasan. El check permanece abierto por composición variable, participación `COMPANY`, reprogramación/Agenda externa, altas/bajas de membresía, adeudos ambiguos y certificación transversal restante. Informe: `docs/pos-automation/runs/2026-09-16T21-32-27-443Z-RV5-P1.md`.
  - **Checkpoint parcial 6, 2026-09-16:** una revisión ya conserva la participación `COMPANY` que el diálogo aprobado muestra bloqueada y envía dentro de `sellerIds`. El servidor exige el mismo `companyId`, ID/código/nombre histórico y una sola fila canónica bloqueada; alta, retiro o sustitución responden `409` antes del token. El reparto igualitario vuelve a incluir empresa y vendedores, pero `REFUND`/`REVISION` proyectan a `Venta`/`PosLegacySaleProjection` únicamente la parte humana, y el evento registra `companyParticipantPreservations` sin reescribir `PosTicketParticipant`. Una venta `100/100` se revisó a `110/110`; las operaciones completas `SALE/REFUND/REVISION` quedaron en `200/200/220`, mientras las proyecciones de vendedor quedaron `100/-100/110`, neto `110`. El rechazo de otra empresa dejó reutilizable la autorización y el replay mantuvo un solo evento. PostgreSQL 16 reconstruyó 46 migraciones y pasó 10/10 POS más 3/3 POS/Scheduler; schemas, tipos, lint, 136 unitarias, builds, formato, manifiesto y diff pasan. El check permanece abierto por composición variable, reprogramación/Agenda externa, altas/bajas de membresía, adeudos ambiguos y certificación transversal restante. Informe: `docs/pos-automation/runs/2026-09-16T21-54-29-655Z-RV5-P1.md`.
  - **Checkpoint parcial 7, 2026-09-16:** el diálogo conserva de forma invisible el `backendLineId` de cada fila aprobada y lo envía como `ticketLineId` opcional; no se añadió ningún control ni cambió el DOM. La transacción valida que las identidades no estén repetidas, no sean ajenas ni se crucen con el orden protegido de paquetes/cortesías. Esto permite revisar dos adeudos del mismo artículo cuando cada uno pertenece a una línea canónica distinta: una línea suelta pasó de `2.00` a `4.00`, la del paquete conservó `3.00`, sus entregas `1.00/2.00` permanecieron ligadas a la unidad correcta, el inventario siguió en `7.00` y `SALE/REFUND/REVISION` dejó proyección neta `700.00`. El replay no duplicó evento ni efectos; una identidad cruzada y dos adeudos que comparten la misma línea devolvieron `409` sin consumir sus tokens. PostgreSQL 16 reconstruyó 46 migraciones y pasó 10/10 POS más 3/3 POS/Scheduler; schemas, contratos, tipos, lint, 136 unitarias, builds, formato y manifiesto pasan. El check permanece abierto por altas/bajas sin identidad, composición variable, reprogramación/Agenda externa y certificación transversal restante. Informe: `docs/pos-automation/runs/2026-09-16T22-10-12-534Z-RV5-P1.md`.
  - **Checkpoint parcial 8, 2026-09-16:** los datasets de ventas ya consumen la última proyección inmutable: detalle usa líneas/vendedores/pagos efectivos, productos vendidos y rentabilidad usan cantidades, importes, costo y metadatos de la revisión, y ventas/desempeño por empleado usan el reparto efectivo. Los tickets sin revisiones conservan la consulta paginada optimizada. Una segunda revisión `100.00 → 120.00 → 160.00` produjo versión 3 con replay único, inventario `6.00`, cinco movimientos `SALE/REFUND/REVISION/REFUND/REVISION` y proyección legacy/bancaria neta `160.00`; recarga, tres reportes, sus exportaciones y conciliación bancaria coincidieron. PostgreSQL 16 reconstruyó 46 migraciones y pasó 10/10 POS más 3/3 POS/Scheduler; el gate de carga sin revisiones pasó 2/2 con 12,000 tickets/24,000 líneas y máximos de página/exportación `1,097.22/2,433.30 ms`. Schemas, tipos, lint, 136 unitarias, builds, formato y manifiesto pasan. El check permanece abierto únicamente para operaciones sin identidad/control aprobado y reprogramación/Agenda HTTP con recuperación durable; no se inventaron decisiones de Producto. Informe: `docs/pos-automation/runs/2026-09-16T22-31-16-196Z-RV5-P1.md`.
  - **Checkpoint bloqueado 9, 2026-09-16:** la reauditoría del árbol fijo `12fb804` confirmó que `TicketEditDialog` sólo ofrece edición general de clienta, participantes, líneas y cobros. `COMPANY` está explícitamente bloqueada; no existe selector de tarjetón/unidad, identidad o composición de paquete/cortesía, ni control de cita, servicio, horario o slot de Agenda. Por ello no hay una operación restante que pueda materializarse sin inferir decisiones de Producto. Las guardas vigentes conservan `409` antes de consumir autorización para dos adeudos sobre una misma línea, altas/bajas/sustituciones ambiguas y Agenda externa sin recuperación durable. No se cambió código, contrato, schema, renderer ni baseline; el check permanece `[ ]`. Informe: `docs/pos-automation/runs/2026-09-16T22-46-34-049Z-RV5-P1.md`.
...
- [x] **RV9-P4 · Desarrollo, post-MVP:** revisar cada retiro R01–R17 contra código, consumidores y evidencia actuales; separar resueltos, parciales, condicionados y piezas que deben conservarse. Ejecutar sólo limpieza sin consumidores o con reemplazo probado; actualizar contratos/tipos/docs y comprobar las apps afectadas. Conservar auditoría, proyecciones compartidas, datos y outboxes; no retirar provider/tablas Agenda durante su ventana de rollback.
  - **Cierre 2026-09-16:** `docs/POS_UI_BACKEND_COMPATIBILITY.md` clasifica individualmente R01–R17 contra el SHA inicial `4bf4dafea066ac580c7b91439509e84088c47240` y REF `12fb804`. R01–R04/R06–R07/R09 tienen reemplazo local demostrado; R05/R08 conservan sólo ramas mock o reductores de presentación explícitos; R10–R13 permanecen condicionados por baseline, RV8, inventario de despliegues o rollback; R14 se conserva por consumidores Envelope/reconciliación; R15–R17 siguen bloqueados por B01–B03. La limpieza ejecutada limita a cuatro dígitos el código personal de un alta nueva en Employees, consistente con REF y `posEmployeeWriteSchema`, sin cortar la lectura 4–12 de credenciales históricas. Tipos/build POS, contrato unitario dirigido, manifiesto de 208 capturas, auditoría de consumidores, formato y diff aprobaron. No cambiaron API, schemas, migraciones, datos, outboxes, provider/tablas Agenda, proyecciones compartidas, baseline, capturador o comparador. Informe: `docs/pos-automation/runs/2026-09-16T22-51-26-680Z-RV9-P4.md`.
...
- [ ] **RV10-P1 · Desarrollo, inmediato:** fijar el SHA candidato y preparar un API de demostración accesible desde Windows y una BD PostgreSQL aislada, o un entorno development con aislamiento de datos de prueba acordado. Aplicar por el flujo autorizado las migraciones del candidato y verificar `/health.release`, `/ready` y CORS. Las 45 migraciones son la evidencia histórica del MVP; comprobar la cadena vigente al ejecutar. Cierre: URL/configuración accesibles, identidad de versión y conexión verificadas.
  - **Checkpoint parcial 2026-09-17:** el SHA candidato quedó fijado en `27b92cacc195f48020033940166a9f5f6460522b`. `backend/api/scripts/prepare-pos-local-demo.sh` prepara de forma opt-in PostgreSQL 16 y el API compilado sin leer `.env`, genera secretos locales con permisos `0600`, exige nombres/estado dedicados y restringe ambos listeners a loopback. Dos arranques comprobaron creación y reentrada: 46/46 migraciones vigentes, `health.release` igual al SHA, `/ready` conectado y CORS permitido para `127.0.0.1:3005`/rechazado para un origin ajeno. La integración HTTP/BD pasó 5/5; también pasaron schemas, tipos, lint, 136/136 unitarias y tipos/build web-Electron del POS. El contenedor `keysar-rv10-p1-pg` y su BD sintética se conservaron detenidos; no se ejecutó el seed general ni se usó `.env` o una BD compartida. El check sigue abierto: esta sesión no dispone de Windows ni de un development autorizado, por lo que no se verificó desde el equipo objetivo una URL accesible. Informe: `docs/pos-automation/runs/2026-09-17T18-28-31-657Z-RV10-P1.md`.
  - **Bloqueo externo 2026-09-17:** sobre el SHA inicial `f7fc12037fe0c8815b78315cf8ac9d9f86c57c44` se revalidaron las guardas y el recurso local conservado: sintaxis Bash correcta, estado/secreto `0600`, PostgreSQL detenido con binding configurado `127.0.0.1:55470` y puertos `4410`/`55470` libres. El host es Fedora Linux nativo y no dispone de Windows/WSL; tampoco se proporcionó un development autorizado. No se ampliaron bindings, desplegó ni mutó la BD. El check permanece abierto hasta comprobar desde el entorno objetivo la URL/configuración, `health.release`, `/ready` y CORS. Informe: `docs/pos-automation/runs/2026-09-17T18-42-54-135Z-RV10-P1.md`.
- [x] **RV10-P2 · Desarrollo, depende de P1:** cargar/provisionar explícitamente un conjunto sintético reproducible: sucursal/perfiles, puestos/empleados, usuarios, credenciales master y operador con permisos distintos, catálogo publicado, precios, existencias, clientas, métodos/bancos/redes/MSI, membresías/cortesías y configuración Scheduler para citas. Reutilizar modelos/servicios; no ejecutar el seed demo general contra una BD compartida. Cierre: todos los pasos de P5 tienen datos y actores válidos, identificados como prueba.
  - **Cierre 2026-09-17:** `backend/api/scripts/provision-pos-local-demo.ts provision|verify` exige confirmación literal, URLs PostgreSQL loopback, nombre/usuario de BD dedicados, dataset explícito, estado dentro de `.pos-runner/` y una base vacía o ya marcada por el mismo dataset. Genera credenciales sólo en un JSON local `0600`, reutiliza hash/fingerprint, permisos, catálogos nacionales y bodega canónicos, y provisiona de forma transaccional/reentrante sucursal, actores master/operador, terminal local sintética, catálogo/precios/existencias, clientas, cobros, membresía/cortesías y Scheduler interno. PostgreSQL 16 con 46/46 migraciones pasó provisionamiento repetido y `verify`; HTTP confirmó ambos logins, 69/35 permisos diferenciados, cuatro ítems, tres métodos y 220 slots. Una segunda BD migrada con datos no marcados fue rechazada antes de crear credenciales. No se ejecutó el seed general, no se usó `.env` ni BD compartida, y no se modificó renderer/baseline. Informe: `docs/pos-automation/runs/2026-09-17T18-46-25-737Z-RV10-P2.md`.
- [ ] **RV10-P3 · Desarrollo, depende de P2:** registrar y activar una terminal para el PO; guardar código/secreto fuera del renderer y del repositorio, entregar credenciales por canal privado y comprobar login, sucursal y permisos. Cierre: acceso online master/operador desde Electron y autorización real mediante los campos aprobados, con rechazo de código incorrecto.
  - **Checkpoint parcial 2026-09-17:** `backend/api/scripts/provision-pos-terminal-local.ts provision|verify` preparó una terminal adicional exclusivamente sintética mediante las rutas canónicas de registro, activación y consulta. Sólo acepta un API HTTP loopback, confirmación literal, dataset explícito y archivos privados dentro de `.pos-runner/`; el secreto se conserva en JSON ignorado `0600` y nunca se imprime. Sobre PostgreSQL 16 local dedicada con 46/46 migraciones, la creación y reentrada dejaron la terminal `ACTIVE` en la sucursal esperada; master/operador iniciaron sesión con 69/35 permisos, un `Código master` incorrecto fue rechazado y el correcto emitió y consumió una autorización ligada a la sesión. La integración POS pasó 10/10, además de schemas, tipos, lint, 136 unitarias, builds API/POS y manifiesto visual 25/10/11/10/208. El check permanece abierto: no se registró la terminal real del PO, no se entregaron credenciales por canal privado y este host no permitió validar Electron/Windows. Informe: `docs/pos-automation/runs/2026-09-17T19-12-25-154Z-RV10-P3.md`.
  - **Bloqueo externo 2026-09-17:** sobre el SHA inicial `1aae14428a90133401726cfe25b1faf45f778042` se confirmó que el host continúa en Fedora Linux sin Windows/WSL, terminal real del PO ni canal privado autorizado. El entorno sintético anterior permanece detenido: API y PostgreSQL sin listeners, contenedor `exited` ligado sólo a `127.0.0.1:55472`, directorio de estado `0700` y archivos sensibles `0600`. La inspección volvió a confirmar el flujo canónico y el manifiesto de referencia 25/10/11/10/208; no se repitieron pruebas funcionales históricas ni se mutó la BD. Sólo cambiaron plan, handoff e informe, el check continúa `[ ]` y no se atribuye validación al PO. Informe: `docs/pos-automation/runs/2026-09-17T19-25-59-775Z-RV10-P3.md`.
- [ ] **RV10-P4 · Desarrollo, depende de P1–P3:** preparar el POS Windows del mismo candidato en `VITE_POS_DATA_MODE=api`, con `VITE_API_URL`/`POS_API_URL` y terminal en main. Revisar inicio y reinicio en Electron y documentar comandos, versiones y parada. El acceso inicial online actual depende de `window.electronAPI.posLogin`; una URL en un navegador limpio no lo reemplaza. Cierre: el PO puede abrir e ingresar a la demo funcional. El instalador y certificación de hardware completos siguen en RV8-P6.
  - **Checkpoint parcial 2026-09-17:** `scripts/verify-pos-windows-demo.mjs check|probe` valida Node/pnpm, SHA completo, modo API sin fixture, igualdad de URLs, presencia redactada de terminal, `health.release`, `/ready` y CORS; `docs/POS_WINDOWS_API_DEMO.md` documenta PowerShell, captura privada, inicio, reinicio y parada sin persistir secretos. La prueba local sobre Linux preparó PostgreSQL 16/46 migraciones, dataset y terminal sintéticos, compiló renderer/main/preload e inició Electron 35.7.5 dos veces. El primer recorrido descubrió que el master intentaba cambiar la terminal a una sucursal vacía antes de hidratarla; `App.tsx` ahora sólo solicita cambio ante una selección explícita. Login y relogin tras reinicio llegaron a `Conteo inicial de inventario` en la sucursal fija, sin cambiar DOM/CSS/baseline. El check sigue abierto: este host no tiene Windows/WSL/Wine, terminal real/canal privado ni acceso del PO. El reinicio Linux también registró una indisponibilidad de la caché `safeStorage`; el acceso online siguió operativo y RV8 conserva esa certificación offline. Informe: `docs/pos-automation/runs/2026-09-17T19-30-40-620Z-RV10-P4.md`.
  - **Bloqueo externo 2026-09-17:** sobre el SHA inicial `2bca5e409c3bdf817f77546a6c69dd7d66af31dc` se confirmó que el host continúa en Fedora Linux sin PowerShell, Windows, WSL ni Wine, y que no se proporcionaron terminal/canal privado o acceso del PO. El estado local anterior permanece privado y detenido: directorio `0700`, archivos sensibles `0600`, contenedor PostgreSQL sintético `exited` y puertos `3005`/`4413`/`55473`/`9223` libres. Las 4/4 pruebas del verificador y el manifiesto 25/10/11/10/208 aprobaron sin iniciar servicios ni mutar datos. Sólo cambiaron plan, handoff e informe; el check continúa `[ ]` hasta ejecutar la guía en el equipo Windows objetivo y demostrar apertura/login del PO. Informe: `docs/pos-automation/runs/2026-09-17T20-09-37-222Z-RV10-P4.md`.
...
- [ ] **RV10-P7 · Desarrollo, antes de entregar:** actualizar `GUIA_PRUEBA_PO_POS_WINDOWS.md` y `PROMPT_CODEX_PRUEBA_PO_POS_WINDOWS.md` para distinguir revisión visual y funcional, indicar el candidato vigente y su relación con `db58fda`, inicio/parada y acceso sin secretos. Publicar evidencia mínima durable con SHA, ambiente/runtime, comparación y resultados; no depender de `/tmp` del equipo original. Cierre: el PO puede reproducir ambas revisiones desde otro equipo y conoce límites del alcance.
  - **Checkpoint parcial 2026-09-17:** ambas guías ahora separan explícitamente fixture visual y recorrido API, fijan el candidato ejecutable `06f9f6f7741b733bda7770e3b67b714daa59a40b` como descendiente de `db58fda`, validan que un HEAD documental posterior no cambie superficies ejecutables, documentan inicio/parada y remiten los secretos sólo a `Read-Host`/canal privado. El baseline durable de `12fb804` se revalidó: índice y 208/208 PNG con hashes correctos, Node `22.23.2`, pnpm `10.0.0`, Playwright `1.60.0` y Chromium `148.0.7778.96`; el manifiesto sigue en 25/10/11/10/208. La comparación candidata 208/208 del 2026-09-09 permanece histórica y sus directorios `/tmp` ya no se presentan como recuperables; no se atribuyó esa comparación al candidato actual. El check sigue abierto: faltan Windows objetivo, API accesible, terminal/canal reales y recorrido del PO, y no existe una nueva comparación completa de `06f9f6f`. Informe: `docs/pos-automation/runs/2026-09-17T20-13-12-758Z-RV10-P7.md`.
  - **Bloqueo externo 2026-09-17:** sobre el SHA inicial `6a6dfe3250dbdb338763c3a41aa32b5183877bd3` se confirmó que el único cambio desde el candidato ejecutable `06f9f6f7741b733bda7770e3b67b714daa59a40b` es documental/estado y que las guías continúan describiendo fielmente lo disponible. El baseline versionado conserva 208/208 hashes válidos y el manifiesto 25/10/11/10/208; las 4/4 pruebas del verificador Windows siguen aprobando. El host es Fedora Linux sin Windows, PowerShell, WSL ni Wine, y no se proporcionaron API/terminal/canal autorizados o acceso del PO. Sólo cambiaron plan, handoff e informe; RV10-P7 permanece `[ ]` y no se promete una revisión funcional no ejecutada. Informe: `docs/pos-automation/runs/2026-09-17T20-22-53-001Z-RV10-P7.md`.
...
- Una tarea puede crear helpers raíz exclusivamente POS con los prefijos `verify-pos-`, `prepare-pos-`, `provision-pos-`, `audit-pos-`, `check-pos-` o `seed-pos-` y extensión `.sh`, `.mjs` o `.ts`; sus pruebas pueden usar `.test.mjs`/`.test.ts`. La plantilla pública `apps/pos/.env.example` puede contener sólo URLs loopback y marcadores no reutilizables. El propio ejecutor, sus pruebas, los scripts generales, otros archivos de entorno, artefactos visuales, dependencias y secretos permanecen fuera de alcance.
...
- `remaining` representa sólo trabajo faltante del criterio asignado. Cuando una sesión cierra `[x]`, todas sus pruebas pertinentes pasan y declara por error seguimientos de otros IDs o requisitos externos, el ejecutor los conserva separadamente como `followUps` y continúa; no aplica esta reconciliación a checks abiertos, pruebas fallidas o cierres sin evidencia.

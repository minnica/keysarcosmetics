...
- RV5–RV7 completan el recorrido online del MVP: Checkout conserva identidad completa de clienta, traduce Visa/Mastercard a IDs autoritativos, soporta pago mixto y usa los campos master aprobados de Producto, Receipts, Settings y X-Report contra tokens de servidor. Receipts consume su autorización una sola vez para revisión o cancelación. RV5-P1 materializa incrementalmente tickets minoristas y adeudos no ambiguos: compensa cobros/proyección legacy, ajusta sólo el inventario físicamente comprometido, conserva originales/entregas append-only y recupera la proyección efectiva al recargar. Revisiones que alterarían membresías, Agenda, paquetes/cortesías, participación de empresa o adeudos duplicados ambiguos siguen bloqueadas con `409` antes de consumir autorización, por lo que RV5-P1 permanece parcial. Membresías/Agenda, reportes, exportaciones, caja y notificaciones continúan conectados a consultas autorizadas. La integración PostgreSQL valida venta mixta, identidad, revisión/cancelación, adeudos/entregas, replay y rechazo de token reutilizado. RV10 comparó 208/208 capturas bajo Node 22.23.2/Chromium 148: tres variaciones de antialiasing (34, 11 y 21 píxeles) quedan bajo `0.000027`, sin modificar el baseline.
- RV5-P1 define `PosTicketRevisionRequestDto` como contrato estricto y materializa el subconjunto soportado dentro de la transacción idempotente existente. `PosTicketEvent.snapshot` registra versión aplicada, actor por relación, antes/después, diferencias e IDs de efectos; `REFUND` compensa los cobros efectivos previos, `REVISION` crea el nuevo cobro y ambas operaciones proyectan a legacy. Para adeudos, `OWED_PRODUCTS` conserva cantidad/estado anterior y posterior e IDs de entregas; nunca permite bajar de lo entregado y reutiliza `PosOwedProduct`/`deliverOwedProduct` sin duplicar movimientos. El ledger registra únicamente el delta físico como `COUNT_ADJUSTMENT`; la lista/detalle selecciona la última proyección y el renderer recarga el ticket después de guardar. No cambian Prisma ni migraciones. Evidencia acumulada: `docs/pos-automation/runs/2026-09-16T18-03-18-954Z-RV5-P1.md` y `docs/pos-automation/runs/2026-09-16T18-35-30-033Z-RV5-P1.md`.

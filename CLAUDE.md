...
- El encabezado y las pantallas de acceso/conteo ofrecen un selector rápido mock de idioma `ES`/`EN`, persistido en `localStorage` mediante `keysar-pos-language`. Actualiza `document.documentElement.lang`, acceso, conteos, metadatos de pantalla, navegación y textos operativos del encabezado. La interfaz permanece siempre en el tema claro `Executive ledger`; al iniciar se elimina cualquier preferencia legacy `keysar-pos-color-mode` y la clase `executive-dark-mode`. El selector no cambia datos ni permisos.

...
- El POS detecta el estado de conexión mediante los eventos `online`/`offline` del renderer. El login, los conteos, la venta y la creación de tickets continúan disponibles sin internet. Cada ticket nuevo o abono creado offline queda marcado `PENDING_SYNC`, con origen local y sin fecha de sincronización; al recuperar internet se cambia automáticamente a `SYNCED`, se registra la hora y se limpia la cola persistente. El acceso exitoso muestra una ventana ejecutiva `Terminal conectada` o `Modo offline activado`, y cada interrupción, recuperación o carga completada genera otro aviso informativo. El encabezado y `Data update` reflejan el estado en vivo, el total pendiente y bloquean actualizaciones manuales mientras no hay conexión. Esta carga es una simulación frontend: todavía no envía información a una API real.
...
> **Modo demo frontend (25 de agosto de 2026):** por instrucción explícita de producto, las rutas visibles de `apps/payroll` operan temporalmente con un estado mock compartido y persistencia exclusiva en `localStorage`. El layout no monta `SessionProvider` ni `SessionGate`, y las pages activas no llaman a `/api/payroll/*`. Backend, Prisma y bases de datos no fueron modificados. La implementación y sus rutas se documentan en `apps/payroll/FRONTEND_MOCK.md`. El código de integración real se conserva sin montar para una reconexión posterior.

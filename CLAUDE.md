...
- **Decisión vigente de presentación y entrega (refinada 2026-09-16):** la única referencia visual aprobada es el árbol completo de `feature/pos` en `12fb8045cc264b565cb6e764d95ad7b2447fbfa1`, reconfirmado por GitHub MCP; `8fd71f3` es sólo histórico. El MVP tiene evidencia técnica del 2026-09-09, pero su cierre no equivale al plan integral ni a un entorno del PO desplegado. `PLAN_RESTAURACION_VISUAL_POS.md` detalla RV10-P1–P8 para preparar Windows/API/BD, datos, credenciales/terminal, recorrido, correcciones y aceptación; RV3-B01 mantiene pendiente el copy de cierre/gasto. RV7-P1 retiró en checkpoint parcial los prompts alias/PIN de corrección/anulación: Cash Manager reutiliza el campo aprobado `Código master`, valida el desbloqueo en servidor y emite un token consumible ligado al gasto para cada mutación; falta comprobar el recorrido HTTP/BD sobre PostgreSQL desechable antes de cerrar el check. Reutilizar al máximo el backend y refactorizar lo necesario detrás de los controles aprobados. RV5/RV6, RV8, escala, hardware, migraciones/limpieza y OP01–OP06 siguen explícitos; SaaS/Websites requieren decisión de alcance. Scheduler interno es el proveedor vigente y las pruebas externas son condicionadas. Validación visual dirigida, sin repetir toda la matriz por fase ni reintroducir pruebas offline al MVP. Evidencia histórica: `docs/POS_MVP_ONLINE_DELIVERY.md`; checkpoint RV7-P1: `docs/pos-automation/runs/2026-09-16T08-55-40-652Z-RV7-P1.md`.

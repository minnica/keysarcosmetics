...
Plan de restauración visual: `PLAN_RESTAURACION_VISUAL_SCHEDULER.md` (6 de septiembre de 2026). Define fases RV0–RV8 para recuperar la presentación aprobada del commit `e9077ddad945325b1a132962ce0c2fcd9ae7f74a` en todo Scheduler, conservando contratos, seguridad y persistencia del backend actual. RV0 quedó implementada con validación visual pendiente: `docs/SCHEDULER_VISUAL_RESTORATION_BASELINE.md` contiene el inventario de rutas/estados/diálogos/acciones, matriz API-permisos, diferencias semánticas, brechas, diagnóstico de infraestructura y el procedimiento aislado. El build de referencia pasó, pero el sandbox bloqueó servidores y Chromium; ejecutar el runner de `apps/e2e/scripts/capture-scheduler-reference.mjs` en un host compatible antes de validar RV0. RV1–RV8 siguen pendientes. No restaurar íntegramente el código mock ni considerar los workspaces API simplificados como referencia visual aprobada. El estado de implementación descrito a continuación sigue siendo el vigente hasta ejecutar esas fases.

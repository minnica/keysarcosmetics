...
- El cierre local de Fase 6 valida schemas, contratos, lint/type-check/build del API, 121 pruebas unitarias en 23 archivos y lint/type-check/build de Scheduler. Las advertencias ya conocidas de imágenes/hooks permanecen no bloqueantes. PostgreSQL 16, integración HTTP/concurrencia, diagnóstico real, provisión y conexión visual de Fase 9 siguen pendientes. Runbook: `docs/SCHEDULER_PHASE_6_ADMINISTRATION.md`.
...
- Salvo el tamaño visual de slots descrito en Agenda, las opciones guardadas en estos paneles todavía no gobiernan reservas, pagos, mensajes ni sitios reales. El backend de Fase 6 ya define alcance `COMMERCE → BRANCH → USER`, permisos, versiones, límite de 64 KiB y rechazo de claves de secretos; la UI se conectará en Fase 9 sin migrar automáticamente documentos de `localStorage`.
...
- La interfaz es responsive desde el inicio: navegación compacta en móvil, tarjetas apiladas, formularios de pantalla completa y tablas que se convierten en bloques legibles. El bloqueo de alto y overflow del viewport pertenece exclusivamente a la ruta `/` de Agenda; Clientes, Reportes, Administración y Configuraciones conservan el crecimiento normal del documento y su scroll vertical de página.
...
- La ruta de Agenda ocupa exactamente el alto visible y bloquea el scroll del documento: header, agenda y navegación permanecen estáticos. El panel de filtros conserva scroll vertical propio y la vista Lista desplaza únicamente su cuerpo, manteniendo fijo su encabezado. El horario mock de los comercios es `09:00–22:00`, por lo que muestra trece filas (`09:00` a `21:00`); `commerce-operating-hours.ts` migra una sola vez el antiguo valor predeterminado persistido `09:00–21:00` a este nuevo cierre, sin alterar configuraciones personalizadas posteriores. La franja `21:00–22:00` se representa mediante bloqueos reales `unavailable` por especialista, exactamente como `10:00–11:00`: es clickeable, abre `Editar bloqueo` y al quitarla deja expuesta la celda normal para crear una reserva o un nuevo bloqueo. El grid no fabrica filas ni tarjetas visuales fuera del horario configurado. Hasta `13` filas visibles se reparten el alto restante mediante `ResizeObserver` dentro de mínimos y máximos por densidad; con `14` o más filas el grid conserva una altura legible y activa scroll vertical interno. Tarjetas, bloqueos y línea de hora actual reutilizan las mismas métricas calculadas para permanecer alineados, y las tarjetas de una hora deben priorizar siempre la visibilidad del nombre aun cuando su fila se reduzca.

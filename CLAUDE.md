...
- Salvo login, sesión, sucursales, permisos, `Employees`, las lecturas de la Fase 2, inventario/bodega de la Fase 3 y tickets/checkout/apartados/adeudos/citas/vouchers de la Fase 4, los demás módulos de `apps/pos` continúan como mocks operativos de React. No escriben todavía jornadas, caja ni reportes en backend. `VITE_POS_DATA_MODE=mock` conserva temporalmente la demo para comparación; esos datos nunca se migran a una BD operativa. La cola frontend de tickets offline en `localStorage` pertenece exclusivamente al modo mock y será sustituida por almacenamiento durable en la Fase 6.

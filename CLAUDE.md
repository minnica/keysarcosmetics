...
- Antes de crear un cliente desde Nueva reserva, Agenda consulta coincidencias autoritativas dentro del alcance. Un teléfono normalizado idéntico bloquea el perfil nuevo y permite usar únicamente el cliente existente; un nombre completo idéntico (nombre y apellidos, ignorando mayúsculas y acentos) abre una confirmación para usar el expediente seleccionado o crear conscientemente un perfil independiente. Las coincidencias parciales de nombre no disparan esta decisión y el backend conserva `PHONE_DUPLICATE` como protección transaccional ante concurrencia.

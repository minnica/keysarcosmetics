...
- La navegación replica el menú operativo de referencia en un sidebar izquierdo ocultable: `Sale`, `Mis ventas`, `Receipts`, `Customers`, `Citas`, `Inventory`, `Catálogo`, `Mov. inventario`, `Settings`, `X-Report`, `Cash manager`, `Close day`, `Employees`, `Competition`, `Websites` y `Data update`. `Sale`, `Mis ventas`, `Receipts`, `Customers`, `Citas`, `Inventory`, `Catálogo`, `Mov. inventario`, `Settings`, `X-Report`, `Close day`, `Employees` y `Data update` tienen vistas mock funcionales; el resto conserva estados de demostración explícitos.
- `Sale` muestra productos y servicios mediante foto, nombre, SKU cifrado, familia, categoría, precio de lista y existencia/agenda. Permite buscar por nombre o SKU y filtrar por familia o categoría. Las imágenes representativas generadas para el catálogo viven en `apps/pos/public/products/`.
- Seleccionar un artículo abre un `Dialog` con cantidad, precio unitario, comentario y total de línea. El precio mínimo se oculta mediante la clave interna `KEYSARCODX` (`0–9`) integrada como sufijo del SKU visible para vendedores (por ejemplo, `KSR-SER-001-CXK`); en las tarjetas de `Sale` aparece debajo del nombre como una única clave `SKU base + sufijo cifrado`, con tratamiento visual estratégico, y nunca como concepto o columna de precio mínimo. El precio de lista se puede incrementar sin límite. La autorización se evalúa contra el ticket completo: una línea puede bajar incluso a `$0` sin código cuando el subtotal de las demás cubre la suma de precios mínimos de todas las piezas. Sólo cuando el total propuesto queda debajo de ese piso combinado se exige el código administrativo mock `2468`. Quitar productos o cambiar cantidades tampoco puede eliminar la cobertura global sin una autorización ya registrada. Las líneas del ticket conservan edición de cantidad, precio o comentario y opción de baja.
- El carrito conserva la variación contra precio mínimo por línea. Al finalizar, el flujo permite buscar un cliente existente por nombre/teléfono o registrar uno nuevo con nombre, apellido, cumpleaños, género, teléfono, WhatsApp, procedencia (`Abordaje`, `Lead`, `Recomendado` o `Redes sociales`), empresa asignada y vendedor fijo obligatorio. El alta nueva también exige agendar un paquete de cortesía con calendario, sucursal y horario mock: `Facial`, `Corporal`, `Doble facial`, `Doble corporal` o `Mixto`. Los paquetes simples generan un registro `COURTESY` y los dobles/mixtos generan exactamente dos; nunca se permiten más de dos cortesías. Cada servicio se agrega al ticket como línea `REGALO` de `$0`. Los leads y clientes de redes sociales conservan su cartera empresarial; los demás clientes nuevos guardan ese vendedor y lo precargan en compras futuras. Los clientes sin propietario activo permanecen en la cartera de `Keysar Cosmetics`. `Settings` determina cuáles campos son obligatorios durante la sesión.
- Antes del cierre se puede abrir un control compacto con icono para aplicar un descuento promocional por porcentaje o importe fijo. El descuento queda limitado automáticamente al margen global disponible: no puede reducir un ticket cubierto por debajo de la suma de mínimos ni profundizar un ticket bajo piso previamente autorizado; ticket, checkout, Receipts y X-Report conservan el importe final en pesos.
- El cierre de ticket funciona como un flujo bloqueado de cuatro pasos (`Cliente → Vendedores → Citas → Cobro`): no permite avanzar mientras el paso vigente esté incompleto. Exige uno o más vendedores, precarga al vendedor ligado al cliente y ofrece `Añadir más vendedores a la venta`; la división puede capturarse por importe o porcentaje. Una clienta con vendedor activo conserva su propietaria sin opción de cambio para el operador común; la reasignación requiere autorización master mock `2468`. Si no tiene propietaria, el segundo paso obliga a elegir una y la agrega automáticamente a la división. El paso `Citas` obliga a responder si la clienta ya cuenta con próxima sesión; si la respuesta es sí, captura servicio, calendario, sucursal y horario y crea un registro `NEXT_SESSION`, y si es no permite continuar sin esa cita adicional. El cobro admite múltiples métodos en el mismo ticket y permite registrar pago completo, apartado con abono o ticket pendiente de cobro. `Settings` incluye una configuración mock protegida para usuario master (`2468`) que permite agregar métodos de pago durante la sesión.
- `Customers` no expone el directorio completo: permite buscar únicamente por teléfono o ingresar una clave mock de vendedor para ver sólo clientes en cuyas ventas participó. Las claves de demostración son `1101`, `2202` y `3303` para los vendedores activos.
...
- `Mis ventas` exige la clave personal de un vendedor activo y filtra estrictamente los tickets donde participa. Permite definir un periodo con calendarios desde/hasta, consultar folio, productos, cliente, teléfono, monto asignado, pagos, apartados, saldo y vista imprimible del ticket. La pestaña `Mis clientes e historial` sólo expone clientes cuyo `ownerId` corresponde al vendedor autenticado, ofrece alternar entre toda su cartera o quienes compraron en el periodo y muestra información de contacto, total histórico, cobrado, pendiente/apartado y acceso a cada ticket. El perfil de cliente lista sus apartados, artículos entregados, historial de abonos y saldo; permite registrar múltiples pagos con los métodos configurados o liquidar, genera para cada abono un folio exclusivo `APT-{timestamp}-{uuid}` y abre su ticket imprimible. Una alerta identifica apartados activos con más de cuatro meses y otra muestra las clientas propias que terminaron su ticket sin próxima facial para seguimiento del vendedor.
- Cada compra genera un folio dinámico de alta entropía (`KSR-{timestamp}-{uuid}`) y abre un ticket final imprimible de 80 mm, monocromático, con logo, empresa, sucursal, dirección, productos, pagos, saldos, políticas y mensaje final. `Settings` permite editar esos textos y decidir por separado si se imprimen nombre/teléfono del cliente y nombre del vendedor. `Close day` muestra antes de cerrar y en su ticket térmico las clientas sin próxima facial, agrupadas bajo cada vendedor participante en la venta.
- `Receipts` agrega el ticket creado a la tabla de sesión y muestra un dashboard de venta por fecha, distribución de métodos de pago y salud de cobranza, además de descuento, monto pagado y saldo pendiente sin revelar margen ni precio mínimo. Incluye calendario, búsqueda por cliente/vendedor/folio y acciones mock de editar datos identificativos, eliminar de la sesión e imprimir nuevamente. El margen contra piso sólo aparece en `X-Report` después de ingresar el código administrativo mock. `Close day` genera un ticket de 80 mm imprimible con productos vendidos, entradas y bajas de inventario, venta por vendedor, corte por método de pago, descuentos, cobro y saldos.
- `Catálogo` usa una tabla en modo listado. El botón `Agregar` separa altas de producto/servicio, familia, categoría y grupo; el formulario de producto selecciona esos tres catálogos desde listas desplegables y captura foto, precios mínimo/lista, existencia, stocks mínimo/máximo y sucursales visibles. Para el SKU pregunta si se genera automáticamente desde tipo + familia + consecutivo disponible (por ejemplo `KSR-KEYSKI-001`) o si se captura un SKU interno manual; al editar conserva inicialmente el modo interno. Los cambios alimentan inmediatamente `Sale` e `Inventory`; vender descuenta existencia. La baja es lógica para el catálogo vigente y no modifica las líneas ya embebidas en tickets históricos.
- `Mov. inventario` conserva existencias independientes para `Polanco`, `Satélite` y `Roma Norte`, y prepara varias sumas, bajas o transferencias antes de aplicarlas como un lote único. Cada renglón identifica explícitamente sucursal/ruta y sólo modifica esa sucursal; una transferencia resta exactamente en origen y suma en destino. Antes de aplicar, el lote muestra cuántos movimientos y productos distintos contiene, permite agregar combinaciones diferentes, editar o quitar cualquier renglón, limpiar únicamente el formulario, limpiar el lote completo y finalmente `Guardar lote`; la ruta y ambos cambios de existencia quedan en la bitácora, sus filtros incluyen `Transferencias` y también aparece en el ticket de `Close day`. Las ventas pagadas descuentan lo disponible en Polanco y, si falta existencia, crean un compromiso de producto por entregar ligado a cliente y ticket; la entrega queda bloqueada hasta que una entrada o transferencia aporte stock suficiente. En un apartado, checkout pregunta qué artículos físicos ya se entregaron y sólo esos se descuentan al crear el ticket; los restantes se intentan descontar al liquidar y cualquier faltante se registra como deuda de producto. `Settings` permite activar, desactivar y agregar motivos de baja; inicia con `Tester`, `Damage`, `Lost` y `Gift`.
- `Data update` muestra por módulo la versión local/disponible, última actualización y estado `Actualizado`, `Requiere actualización` o `Actualizando`. Permite ejecutar manualmente un módulo o todos los pendientes; la descarga y el progreso son simulados en memoria y no consumen servidor.
...
└── src/renderer/src/
    ...
    └── components/
        ...
        ├── DataUpdateView.tsx               → actualización manual mock por módulo

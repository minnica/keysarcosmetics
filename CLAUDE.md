...
- Cada checkpoint abre una sesión nueva, actualiza plan/handoff/informe, verifica cambios y hace commit/push exclusivamente a `feature/pos-frontend-clean`. Requiere checkout limpio y sincronizado. Las consultas Git remotas reintentan timeouts transitorios; un `push` con timeout sólo se acepta si el SHA esperado ya está confirmado en la rama remota. `--recover-push` recupera una publicación fallida sin repetir la implementación.
- Los helpers raíz creados por una tarea automática deben ser exclusivamente POS y llamarse `scripts/{verify,prepare,provision,audit,check,seed}-pos-*` con extensión `.sh`, `.mjs` o `.ts`; sus pruebas pueden usar el sufijo `.test.mjs`/`.test.ts`. El runner, sus pruebas y los scripts generales permanecen protegidos. Los helpers exclusivos del API deben preferir `backend/api/scripts/`. La allowlist admite únicamente `apps/pos/.env.example` como plantilla pública; todo `.env` real, otros ejemplos y secretos siguen bloqueados.
- El resultado `failed_unrelated` sólo aplica cuando una suite amplia falla fuera del criterio/archivos de la tarea, las pruebas dirigidas aprobaron y el informe identifica causa y futura asignación. Un `failed` pertinente conserva cambios y detiene el ejecutor.
- En la salida automática, `remaining` contiene sólo trabajo no satisfecho del check actual. Si una tarea se declara `completed`, marca su check, aprueba las verificaciones y enumera por error pendientes de otros IDs/decisiones externas, el ejecutor los conserva como `followUps` y continúa; no normaliza pruebas fallidas ni un check todavía abierto.
...
- **Decisión vigente de presentación y entrega (refinada 2026-09-16):** la única referencia visual aprobada es el árbol completo de `feature/pos` en `12fb8045cc264b565cb6e764d95ad7b2447fbfa1`, reconfirmado por GitHub MCP; `8fd71f3` es sólo histórico. El MVP tiene evidencia técnica del 2026-09-09, pero su cierre no equivale al plan integral ni a un entorno del PO desplegado. `PLAN_RESTAURACION_VISUAL_POS.md` detalla RV10-P1–P8 para preparar Windows/API/BD, datos, credenciales/terminal, recorrido, correcciones y aceptación; RV3-B01 mantiene pendiente el copy de cierre/gasto. RV7-P1 retiró los prompts alias/PIN de corrección/anulación: Cash Manager reutiliza el campo aprobado `Código master`, valida el desbloqueo en servidor y emite tokens consumibles ligados al gasto para cada mutación. La integración HTTP/BD sobre PostgreSQL 16 desechable ya comprueba permiso `CASH_MANAGE`, persistencia, importes/compensaciones, idempotencia y rechazo de reutilización; B01 no cambió. Reutilizar al máximo el backend y refactorizar lo necesario detrás de los controles aprobados. RV5/RV6, RV8, escala, hardware, migraciones/limpieza y OP01–OP06 siguen explícitos; SaaS/Websites requieren decisión de alcance. Scheduler interno es el proveedor vigente y las pruebas externas son condicionadas. Validación visual dirigida, sin repetir toda la matriz por fase ni reintroducir pruebas offline al MVP. Evidencia histórica: `docs/POS_MVP_ONLINE_DELIVERY.md`; cierre RV7-P1: `docs/pos-automation/runs/2026-09-16T09-28-04-169Z-RV7-P1.md`.
- RV0 fijó el backend de partida en `e57b9ee9d48519cb762ca2d9294496abe93ce81e` y confirmó `12fb804` mediante GitHub MCP. `apps/e2e/pos-visual/reference-fixture.json` conserva hashes, runtime y reloj; `reference-scenarios.json` inventaría las 25 pantallas, 10 secciones de Settings, 11 reportes, 10 vistas de bodega y 208 capturas programadas. `capture-pos-reference.mjs` valida la fuente completa, bloquea red/API/BD, redacta claves históricas y publica evidencia sólo de forma atómica; `compare-pos-baseline.mjs` compara por píxel con tolerancia cero por default. Guía: `docs/POS_VISUAL_BASELINE.md`.
- La referencia aislada pasó type-check y build Vite sin correcciones. Tras reiniciar Codex con Full Access, `pnpm pos:visual:capture:reference` completó 208/208 capturas con Node `22.23.2`, pnpm `10.0.0`, Playwright `1.60.0` y Chromium `148.0.7778.96`; `capture-index.json` registra el SHA y hashes individuales verificados. Los intentos previos bajo sandbox restringido no publicaron evidencia parcial. La validación Electron y los tamaños físicos de terminal siguen pendientes como gates posteriores de hardware/Operación, no bloquean RV0.
- RV1 auditó las 25 pantallas contra el renderer, los 142 handlers POS, contratos, entidades, offline, impresión/exportación y consumidores de Envelope, Payroll y Scheduler. La matriz en `docs/POS_UI_BACKEND_COMPATIBILITY.md` registra 101 capacidades y 17 retiros condicionados. No se autoriza borrar nada hasta que exista reemplazo, migración de consumidores, preservación de datos y aceptación. Employees/autorizaciones no deben añadir alias o PIN visibles ni ocultar altas/roles; My Account SaaS y Websites conservan su visual pero permanecen fuera del backend POS. Los conflictos de copy operativo, código delegado y edición de pedido de bodega deben resolverse en su fase y no bloquean iniciar RV2.
- RV2 restauró la presentación del renderer sin sustituir CSS/assets ni main/preload/IPC/offline. Recuperó los formularios aprobados de sucursal y cancelación, los módulos imprimibles de Employees, la composición de Diseño de ticket y la variante `DatePicker` en español con navegación mes/año para cumpleaños, sin cambiar el default compartido. La matriz Chromium capturó 208/208 estados: 205 fueron idénticos con tolerancia cero y tres variaciones de antialiasing (34, 21 y 11 píxeles de 1,296,000) quedaron dentro del umbral medido `0.000027`, para 208/208 `PASS`. El modo visual exige explícitamente `mock + VITE_POS_VISUAL_FIXTURE=1`; el build normal continúa en API y nunca cae a fixtures. Cancelación delegada e identidad comercial siguen como límites funcionales de fases posteriores. Evidencia: `docs/POS_RV2_PRESENTATION_RESTORATION.md`.
- RV3 conectó los formularios aprobados a login/sesión/terminal, autorizaciones por código, Employees canónico, permisos/alcance, delegación master auditable, autoservicio de credencial, Clock In/Out, apertura/conteos/cierre y salida sin Close Day. Los cambios de credencial, rol, permisos, alcance, delegación, estado o sucursal de terminal incrementan versión y revocan inmediatamente sesiones y autorizaciones afectadas. `PosDelegatedMasterCode` nunca expone hash/huella/código y `managedByDelegation` evita que una revocación quite perfiles master nativos. Las 44 migraciones, 17 pruebas HTTP habilitadas, 135 unitarias, builds/type-check/lint y 208 comparativas visuales pasan. B01 sigue abierto: no cambiar el texto aprobado “mock/simulación” de Close Day sin copy de Producto. Evidencia: `docs/POS_RV3_ACCESS_WORKFORCE_DAY.md`.
- RV4 conectó catálogo, clientas/cartera, proveedores, inventario, bodega y Settings a API/Prisma. La migración aditiva `20260909010000_pos_rv4_catalog_inventory_settings` amplía catálogo, clientes, paquetes, ticket, listas y pedidos sin destruir históricos; las revisiones de bodega conservan folio/versiones append-only. Costos y sucursales se filtran en servidor, y las acciones sensibles consumen autorización Master de propósito/sesión/terminal una sola vez. Las 45 migraciones se reconstruyeron en PostgreSQL 16; pasan 18 pruebas HTTP habilitadas, 135 unitarias, schemas/type-check/lint/build y la matriz Chromium 208/208 sin cambiar el baseline. Evidencia: `docs/POS_RV4_CATALOG_CUSTOMERS_INVENTORY_SETTINGS.md`.
- RV5–RV7 completan el recorrido online del MVP: Checkout conserva identidad completa de clienta, traduce Visa/Mastercard a IDs autoritativos, soporta pago mixto y usa los campos master aprobados de Producto, Receipts, Settings y X-Report contra tokens de servidor. Receipts consume su autorización una sola vez para revisión o cancelación. RV5-P1 materializa incrementalmente tickets minoristas, adeudos identificables por su fila persistida, membresías cuya composición no cambia, participantes `COMPANY` preservados, paquetes/cortesías sin sustitución y revisiones que conservan una cita Scheduler interna coherente: compensa cobros/proyección legacy, ajusta inventario comprometido y mantiene originales, entregas, asistencias, servicio, horario, capacidad, atribución comercial, `PosPackage` y `PosCourtesy`. Reprogramación/Agenda externa, altas/bajas/sustituciones de tarjetón, paquete o cortesía, cambio de identidad `COMPANY` o dos adeudos que comparten una misma identidad de línea siguen bloqueados con `409` antes de consumir autorización, por lo que RV5-P1 permanece parcial. La integración PostgreSQL valida venta mixta, identidad, revisión/cancelación, adeudos/entregas —incluido el mismo artículo en una línea suelta y otra de paquete—, membresía con sesión consumida, segunda revisión, preservación de Scheduler/capacidad/paquetes/cortesías/empresa, proyección exclusiva de la parte de vendedores, reportes/exportaciones efectivos, conciliación bancaria, X-Report, replay y rechazo seguro. RV10 comparó 208/208 capturas bajo Node 22.23.2/Chromium 148: tres variaciones de antialiasing (34, 11 y 21 píxeles) quedan bajo `0.000027`, sin modificar el baseline.
- RV5-P1 define `PosTicketRevisionRequestDto` como contrato estricto y materializa el subconjunto soportado dentro de la transacción idempotente existente. Cada producto puede aportar `ticketLineId` opcional: el renderer lo deriva de la fila aprobada ya cargada (`backendLineId`) sin mostrar otro campo, y el servidor rechaza IDs repetidos, ajenos o cruzados antes de consumir autorización. `PosTicketEvent.snapshot` registra versión aplicada, actor, antes/después, diferencias e IDs de efectos; `REFUND` compensa el cobro efectivo y `REVISION` proyecta el nuevo. Para adeudos, `OWED_PRODUCTS` conserva cantidades, estados y entregas, y el ledger registra sólo el delta físico; si un artículo aparece en varias líneas, cada adeudo se empareja por `ticketLineId` y sólo se admite cuando esa identidad es única. Para membresías, `MEMBERSHIPS` bloquea las filas y conserva asistencias/sesiones; `PosMembershipRevisionProjection` mantiene importe/clienta efectivos por versión sin modificar los snapshots protegidos, mientras vendedor y activación reutilizan `PosMembershipSellerChange` y `PosMembershipStatusChange`. Para citas internas, bloquea `PosAppointment` y `SchedulerAppointment`, valida identidad/servicio/slot/capacidad y registra `appointmentPreservations` sin mutar Agenda; una cita externa o inconsistente falla antes del token. Para paquetes y cortesías, reconstruye la identidad desde las líneas persistidas, bloquea `PosPackage`/`PosCourtesy`, excluye `GIFT` de cotización e inventario y registra `packagePreservations`/`courtesyPreservations`; sólo permite composición, cantidades y precios idénticos, y exige que el paquete siga publicado y vigente. Para participación comercial, usa el `companyId` bloqueado que ya envía el diálogo aprobado, conserva ID/código/nombre del participante original, recalcula el reparto igualitario y registra `companyParticipantPreservations`; alta, retiro o sustitución fallan antes del token, y `Venta`/`PosLegacySaleProjection` reciben sólo la fracción humana tanto al compensar como al volver a proyectar. `appliedTicketRevisionSnapshot` sirve como lector común para ticket y reportes: detalle, productos/rentabilidad, vendedores/desempeño y formas de pago usan líneas, costos, metadatos, reparto e IDs de cobro de la última revisión; conciliación bancaria conserva la secuencia completa con signos. Los periodos sin revisiones mantienen la consulta paginada optimizada y su gate de carga. Lista, exportación, cierre mensual y X-Report leen la última proyección. Evidencia acumulada: `docs/pos-automation/runs/2026-09-16T18-03-18-954Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T18-35-30-033Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T18-49-58-408Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T21-13-03-305Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T21-32-27-443Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T21-54-29-655Z-RV5-P1.md`, `docs/pos-automation/runs/2026-09-16T22-10-12-534Z-RV5-P1.md` y `docs/pos-automation/runs/2026-09-16T22-31-16-196Z-RV5-P1.md`.
- RV5-P2 completa el historial canónico de entregas en el contrato aditivo `PosOwedProductDto.deliveries`: cada evento expone folio, fecha operativa/instante, cantidad, credencial/nombre del actor y movimiento de inventario desde `PosOwedProductDelivery`/`Line`. `ticketInclude` es la única inclusión reutilizada por lista, detalle y bootstrap offline; el campo permanece opcional sólo para leer bootstraps cifrados anteriores sin cambiar `schemaVersion`. Receipts, expediente de Customers y seguimiento de Inventory muestran esos eventos dentro de sus controles aprobados y el adaptador ya no sustituye el historial persistido por `[]`. Evidencia: `docs/pos-automation/runs/2026-09-16T17-12-01-729Z-RV5-P2.md`.
- RV6-P1 conecta la tarjeta aprobada `CIERRE MENSUAL · TOP 3 VENDEDORES` al snapshot canónico del último mes y alcance master. `createMembershipClosure` conserva el advisory lock y sólo agrega una versión cuando conteo, importe o ranking cambian; una solicitud idéntica devuelve el mismo snapshot. El perfilamiento visible continúa en `/memberships/:id/profile` y su replay HTTP/BD está comprobado. `12fb804` no contiene controles de reasignación de vendedor ni estado manual: `/seller` y `/status` permanecen sin invocación desde el renderer hasta que Producto defina esa superficie; sus historiales visibles y las transiciones automáticas por liquidación, asistencia y cancelación se conservan. Evidencia: `docs/pos-automation/runs/2026-09-16T19-15-50-093Z-RV6-P1.md`.
- RV6-P2 certifica el proveedor Scheduler interno en PostgreSQL: disponibilidad y último profesional concurrente, clienta nueva/existente, cortesía simple/doble, próxima sesión, cancelación, asistencia, no-show y reintentos. En modo interno, el servicio de próxima sesión se deriva del `SchedulerServiceProfile` codificado por el slot y debe coincidir al confirmar; no se usa el artículo `MEMBERSHIP` como servicio. `ATTENDED` consume el beneficio reservado exactamente una vez, mientras `CANCELED` y `NO_SHOW` lo liberan sin incrementar `usedSessions`. `SchedulerAppointment` sigue siendo la autoridad y `PosAppointment`/`AgendaReservation` son proyecciones conciliadas. Evidencia: `docs/pos-automation/runs/2026-09-16T19-37-14-982Z-RV6-P2.md`.
- RV7-P2 certifica técnicamente `/tickets`, `/reports/:key` y `/exports/:key` con datos sintéticos locales: 1/10/20/30 sucursales, 400 tickets por sucursal, 366 días, tres históricas inactivas, 12,000 tickets y 24,000 líneas máximas. El gate opt-in `backend/api/src/pos-report-load.integration.test.ts` se habilita sólo con `RUN_POS_REPORT_LOAD_TESTS=true`; por defecto no carga datos. Umbrales configurables: `POS_REPORT_LOAD_SCREEN_PAGE_MAX_MS=2000`, `POS_REPORT_LOAD_SCREEN_COMPLETE_MAX_MS=120000`, `POS_REPORT_LOAD_EXPORT_MAX_MS=30000` y `POS_REPORT_LOAD_TICKETS_PER_BRANCH=400`. La corrida de cierre concilió los folios hidratados por la pantalla, el reporte paginado, el archivo completo, los 13 datasets, costos y auditoría; a 30 sucursales midió 6.64 s de hidratación, 7.10 s de reporte y 2.40 s de exportación. Son umbrales locales de ingeniería, no SLA ni aceptación operativa. Evidencia: `docs/pos-automation/runs/2026-09-16T20-00-55-973Z-RV7-P2.md`.
- RV9-P3 ensaya de forma reproducible la actualización desde el snapshot sintético de 45 migraciones hasta las 46 vigentes. `scripts/verify-pos-migration-recovery.sh` exige tres PostgreSQL locales, distintas y vacías, con `pos_upgrade` en el nombre, schema `public` y confirmación `EPHEMERAL_ONLY`; acepta clientes PostgreSQL nativos o un contenedor Podman explícito y nunca crea, vacía o elimina bases. El fixture `backend/api/scripts/pos-migration-recovery-fixture.ts` genera secretos/hash localmente y preserva ticket, pago, membresía, auditoría y una `PosSyncOperation` pendiente. El gate crea un respaldo custom, verifica su contenido, restaura dos copias, aplica `prisma migrate deploy` y comprueba consumidores, tabla/trigger append-only y ledger de migraciones. No usarlo con datos compartidos ni como sustituto de backup/PITR operativo. Evidencia: `docs/pos-automation/runs/2026-09-16T20-27-47-323Z-RV9-P3.md`.
- RV9-P4 reaudita R01–R17 en `docs/POS_UI_BACKEND_COMPATIBILITY.md`. Los reemplazos locales R01–R04/R06–R07/R09 están resueltos; R05/R08 conservan ramas mock o cálculo de presentación sin ser fuentes persistentes; R10–R13 requieren inventario de despliegues, RV8 o fin de rollback; R14 se conserva por Envelope/reconciliación; R15–R17 siguen sujetos a B01–B03. El alta nueva de Employees limita el código personal a los cuatro dígitos de `12fb804` y `posEmployeeWriteSchema`, pero login/autorización/asistencia mantienen lectura 4–12 para rotar credenciales históricas. No retirar mocks, cola local, providers/tablas Agenda, proyecciones legacy, outboxes ni datos por esta auditoría. Evidencia: `docs/pos-automation/runs/2026-09-16T22-51-26-680Z-RV9-P4.md`.
- RV10-P1 dispone de preparación local reproducible mediante `backend/api/scripts/prepare-pos-local-demo.sh up|verify|status|down`. El helper exige `POS_DEMO_CONFIRMATION=LOCAL_SYNTHETIC_ONLY`, usa sólo PostgreSQL 16 ya disponible, estado ignorado bajo `.pos-runner/`, secretos generados localmente y listeners loopback; no lee `.env`, no ejecuta seeds, no descarga imágenes y conserva la BD al detenerse. Fija `RELEASE_SHA`, aplica las 46 migraciones vigentes, compila el API y verifica `health.release`, `/ready`, CORS y bindings. El listener API acepta `HOST` y conserva `0.0.0.0` como default; la preparación fija `127.0.0.1`. La prueba local pasó, pero RV10-P1 permanece parcial hasta comprobar la URL desde Windows o un development aislado autorizado. Evidencia: `docs/pos-automation/runs/2026-09-17T18-28-31-657Z-RV10-P1.md`.
- RV10-P2 añade `backend/api/scripts/provision-pos-local-demo.ts provision|verify`. Requiere confirmación `LOCAL_SYNTHETIC_ONLY`, dataset explícito, URLs coincidentes hacia PostgreSQL loopback, nombre `keysar_pos_demo[_sufijo]`, usuario dedicado y archivo de credenciales dentro de `.pos-runner/`. Rechaza una BD con datos previos sin su marcador auditable, escribe secretos sólo en JSON local `0600`, carga todo transaccionalmente y es reentrante incluso ante historiales append-only. Reutiliza seguridad POS, permisos migrados, bodega y bancos/redes/MSI canónicos; no usa seed general. Provisiona sucursal/perfiles, actores master/operador con permisos distintos, terminal local sintética, catálogo/precios/inventario, clientas/pagos, membresía/cortesías y Scheduler interno. `verify` comprueba hashes, permisos, cantidades, configuraciones y marcador sin exponer secretos. Evidencia: `docs/pos-automation/runs/2026-09-17T18-46-25-737Z-RV10-P2.md`.
- RV10-P3 añade `backend/api/scripts/provision-pos-terminal-local.ts provision|verify` para ensayar registro/activación sin convertir la terminal sintética en la del PO. Exige confirmación `LOCAL_SYNTHETIC_ONLY`, API HTTP loopback, dataset coincidente y archivos privados bajo `.pos-runner/`; bloquea redirecciones y escapes por enlaces simbólicos. Usa el login administrativo sintético sólo para las rutas canónicas de alta/estado/rotación, guarda el secreto de única lectura en JSON `0600` y valida después master/operador, sucursal, separación de permisos, rechazo de Código master incorrecto y emisión/consumo del correcto. No imprime secretos ni usa seed general. El check permanece parcial hasta ejecutar Electron/Windows con la terminal real y entregar credenciales por canal privado. Evidencia: `docs/pos-automation/runs/2026-09-17T19-12-25-154Z-RV10-P3.md`.
- RV10-P4 añade `scripts/verify-pos-windows-demo.mjs check|probe` y `docs/POS_WINDOWS_API_DEMO.md`: comprueba versiones, SHA/HEAD, modo API sin fixtures, URLs coincidentes, configuración redactada de terminal, release/readiness y CORS, y documenta PowerShell/inicio/reinicio/parada sin persistir secretos. El primer login API ya no intenta cambiar la terminal mientras `requestedBranch` todavía está vacío; conserva la sucursal autoritativa devuelta por main y sólo usa el flujo de cambio existente ante una selección explícita. En Linux, Electron 35.7.5 abrió, inició sesión y repitió el login tras reinicio contra PostgreSQL/API sintéticos; no cambió DOM/CSS/baseline. El check permanece parcial hasta repetir en Windows con terminal/canal reales y acceso del PO. La caché `safeStorage` no pudo abrirse en el segundo arranque Linux, pero el acceso online siguió operativo; la certificación offline/instalable continúa en RV8-P6. Evidencia: `docs/pos-automation/runs/2026-09-17T19-30-40-620Z-RV10-P4.md`.
- RV10-P7 actualiza `GUIA_PRUEBA_PO_POS_WINDOWS.md` y `PROMPT_CODEX_PRUEBA_PO_POS_WINDOWS.md`: el candidato ejecutable documentado es `06f9f6f7741b733bda7770e3b67b714daa59a40b`, descendiente del MVP histórico `db58fda`; una revisión desde un commit documental posterior debe comprobar que no cambiaron `apps/pos`, API, contratos compartidos, manifests o lockfile. El modo visual usa exclusivamente `mock + VITE_POS_VISUAL_FIXTURE=1` y no acredita persistencia; el modo API permanece condicionado a la guía `docs/POS_WINDOWS_API_DEMO.md`, SHA/health/CORS, terminal activa y credenciales por canal privado. El baseline durable conserva 208 PNG e índice verificados de `12fb804`; la comparación 208/208 del 2026-09-09 es un resumen histórico porque sus artefactos candidato/diff estaban en `/tmp`, y no se atribuye a `06f9f6f`. RV10-P7 continúa parcial hasta ejecutar Windows/API con el PO y producir la evidencia faltante. Informe: `docs/pos-automation/runs/2026-09-17T20-13-12-758Z-RV10-P7.md`.
...
- `reconcile-pos-pilot.ts` ahora comprueba activaciones/consumos, Agenda, MSI, participantes empresa, dependencias offline y fuentes pendientes de reportes. `POS pilot gate` admite una segunda pasada multi-sucursal y exige confirmación de Agenda, Finanzas, Operación y Producto. El cierre local valida schemas, lint/build del API, type-check/build Vite del POS y 84 pruebas unitarias. La prueba Electron interactiva, PostgreSQL 16 desde cero, integración HTTP, sandbox de Agenda y los pilotos reales siguen siendo puertas obligatorias antes de producción. Guía: `docs/POS_OFFLINE_SECOND_PILOT.md`.
- La Fase 13 de `PLAN_BACKEND_POS.md` quedó completada en repositorio el 2026-09-04 mediante la migración aditiva `20260904040000_add_pos_reporting_indexes`. Sólo agrega índices para consultas canónicas de membresías, pagos, citas y periodos; no crea datos operativos, no se aplicó a development o producción y ambos schemas Prisma deben permanecer sincronizados.
- `backend/api/src/routes/pos-report.routes.ts` publica `CUSTOMER_SOURCE_MONTHLY`, `BANK_RECONCILIATION` e `INVENTORY_COUNTS` para pantalla y exportación desde el mismo constructor. Procedencia agrupa por `CustomerSource.id` y cliente único; conciliación genera una fila por `PosPayment`, distingue venta/abono/liquidación/compensación y suma la venta comercial una sola vez por ticket; conteos conserva apertura, movimiento, existencia y cierre por sucursal antes de consolidar. Todos los periodos declaran `America/Mexico_City`.
- `PosDataScope` separa alcance operativo activo de alcance histórico explícito. Dashboard, tickets/Receipts/Mis ventas, Customers, Citas, Membresías, Inventory, Cash Manager, X-Report y Reports materializan sucursales antes de consultar. Una sucursal inactiva asignada sigue disponible para históricos, pero no para nuevas ventas, reservas ni movimientos. Las membresías se enlazan exclusivamente por `customerId` y `purchaseTicketId`; el contrato reporta estrategia `CANONICAL_IDS` y no ejecuta fallback silencioso por teléfono/nombre.
- `/api/pos/exports/:key` devuelve todo el conjunto filtrado, no sólo la página visual, y cada exportación agrega `AuditLog` con actor, terminal, periodo, filtros seguros, sucursales, acceso a costos y filas. La exportación de membresías aplica la misma auditoría. Los datasets incluyen `scope`; el renderer agrega ese alcance y los `branch_id` autorizados a PDF/XLSX.
- El cierre local de la Fase 13 valida schemas sincronizados/válidos, lint y type-check de types/API client/API/POS, build del API, build Vite de renderer/main/preload y 83 pruebas unitarias. La reconstrucción desde cero, igualdad HTTP pantalla/exportación y volumen real con 30 sucursales sobre PostgreSQL 16 efímero siguen pendientes porque el workspace no dispone de base desechable; son obligatorios antes de desplegar. Contrato y operación: `docs/POS_REPORTING_SCALE.md`.
- La Fase 12 de `PLAN_BACKEND_POS.md` quedó completada en repositorio el 2026-09-04 mediante la migración aditiva `20260904030000_add_pos_commercial_rules`. Agrega catálogos versionados de bancos/redes/plazos, snapshots comerciales de `PosPayment`, productos/paquetes de cortesía versionados, identidad de empresa, `PosTicketParticipant` y eventos de transferencia de cartera. Sólo siembra los 54 bancos ABM revisados el 2026-09-04, Visa/Mastercard, los plazos 1/3/6/9/12/18/24 y la identidad técnica `EMPRESA-001`; no crea datos operativos ni modifica `Bank` de Payroll. Ambos schemas Prisma deben permanecer sincronizados y la migración no se aplicó a development o producción.
- `backend/api/src/routes/pos-commercial.routes.ts` administra los catálogos, cortesías, empresa y baja/reactivación de vendedores con permisos POS. La baja serializable transfiere sólo cartera vigente a empresa y la reactivación no la restaura. Los tickets empresariales exigen participante `COMPANY`; nombres, número e importes quedan en snapshots append-only y la proyección `Venta` sólo recibe la porción humana, sin vendedor ficticio ni comisión duplicada.
- Los pagos de tarjeta exigen tipo, red y banco activos, autorización de cuatro dígitos y plazo vigente sólo para crédito; transferencia admite banco sin tipo/red. Venta, pago mixto, abono/liquidación y compensación conservan estas dimensiones. El contrato no admite PAN/CVV/banda y rechaza números de tarjeta con Luhn en campos libres. La integración completa del estado del renderer y bootstrap offline permanece en la Fase 14. Contrato y operación: `docs/POS_COMMERCIAL_RULES.md`.
- El cierre local de la Fase 12 verifica schemas sincronizados/válidos, migración aditiva segura, lint y type-check de types/API client/API/POS, 75 pruebas unitarias, build del API y build Vite de renderer/main/preload. El empaquetado instalable no pudo descargar Electron por la red restringida. La reconstrucción e integración/concurrencia sobre PostgreSQL 16 efímero quedan pendientes porque Podman no puede crear su runtime en este workspace; son obligatorias antes de desplegar.
- La Fase 11 de `PLAN_BACKEND_POS.md` quedó implementada en repositorio el 2026-09-04 mediante la migración aditiva `20260904020000_add_pos_agenda_integration`. Agrega `Customer.externalClientId`, recursos/slots, reservas, eventos de sincronización, snapshots externos de citas y compensaciones de asistencia; no crea seeds ni datos operativos. Ambos schemas Prisma deben permanecer sincronizados y la migración no se aplicó a development o producción durante la implementación.
- `backend/api/src/services/agenda-adapter.ts` es el único cliente de Agenda CRM. `AGENDA_API_URL`, `AGENDA_API_TOKEN`, `AGENDA_WEBHOOK_SECRET` y el timeout viven sólo en servidor. El POS consulta `/api/pos/agenda/availability`, envía `agendaSlotId` en checkout y nunca recibe credenciales externas. Cada reserva vuelve a validar versión/capacidad; las dobles usan dos lugares de cabina `DOUBLE` o una saga de slots consecutivos con compensación durable.
- Una venta con cita persiste primero la intención, hace `upsert` de clienta/reserva con claves derivadas del `Idempotency-Key` y después confirma el ticket local. Un fallo remoto impide crear el ticket; un fallo del commit deja `CANCEL_PENDING` para `pnpm --filter @cosmetics/api pos:agenda:worker`. Editar clienta o cancelar ticket genera eventos reintentables y Citas expone una cola redactada según sucursales autorizadas.
- `/api/pos/agenda/webhooks` verifica HMAC sobre body crudo y ventana de cinco minutos. `eventId` y versión hacen idempotentes y ordenables `ATTENDED`, `CANCELED` y `NO_SHOW`; sólo asistencia consume membresía. Una contradicción queda `CONFLICT` y requiere autorización master `AGENDA_ATTENDANCE_CORRECTION`; la corrección agrega `PosMembershipAttendanceCorrection` sin modificar la asistencia append-only. Próximas sesiones de membresía respetan vigencia, cartera y cualquier sucursal autorizada.
- El cierre local de la Fase 11 valida contrato del adaptador, firma/replay, reservas dobles parciales y agrupación mediante pruebas unitarias. La puerta real contra sandbox de Agenda y PostgreSQL 16 efímero sigue pendiente porque el workspace no dispone de URL/token ni servidor de BD. No habilitar Agenda en un ambiente hasta completar esos casos; el contrato operativo está en `docs/POS_AGENDA_INTEGRATION.md`.
- La Fase 10 de `PLAN_BACKEND_POS.md` quedó completada en repositorio el 2026-09-04 mediante la migración aditiva `20260904010000_add_pos_memberships`. Agrega `MEMBERSHIP`, condiciones versionadas, tarjetones por unidad, asistencias, historiales de vendedor/estado y cierres/rankings mensuales sin crear seeds ni datos operativos. Ambos schemas Prisma deben permanecer sincronizados; la migración no se aplicó a development ni producción durante la implementación.
- Una venta de membresía genera SKU `MEM-######`, conserva precio, IVA y visibilidad, exige unidades enteras y nunca toca inventario físico. Cada unidad crea un `PosClientMembership` y folio únicos con snapshots de cliente, términos, importe, sucursal y vendedor. `COMPLETED` activa en el commit; `LAYAWAY` queda pendiente y se activa una sola vez al liquidarse; cancelaciones o devoluciones totales agregan un cambio `CANCELED` sin borrar historial.
- `/api/pos/memberships*` ofrece listado paginado, detalle, seguimiento, asistencia, perfil, cambios de vendedor/estado, exportación y cierres versionados. Cada acceso exige grant específico y autorización personal `MEMBERSHIPS_ACCESS`; el servidor aplica cartera vigente y sucursales antes de consultar. Master debe materializar el conjunto explícito de sucursales para listados, descargas y cierres.
- El consumo de sesiones usa bloqueo de fila y unicidad por cita/número de sesión: sólo `ATTENDED` consume y dos confirmaciones concurrentes convergen en una asistencia. Los estados de firma quedan reservados como `PENDING/SIGNED/NOT_REQUIRED`; referencias, consentimiento, hash y metadatos de evidencia son privados y no forman parte del DTO. La integración real de Agenda/webhooks corresponde a la Fase 11. El contrato operativo está en `docs/POS_MEMBERSHIPS.md`.
- El cierre local de la Fase 10 verificó migración segura, schemas sincronizados/válidos, type-check de types/API client/API/POS, lint y build del API, build Vite de renderer/main/preload y 56 pruebas unitarias. `pos-memberships.integration.test.ts` quedó descubierto por el runner y compila el gate transaccional, pero no se ejecutó porque el workspace no tiene `DATABASE_URL` y Podman no puede crear su runtime; debe correr con PostgreSQL efímero antes de desplegar.
- La Fase 9 de `PLAN_BACKEND_POS.md` quedó implementada en repositorio el 2026-09-03 mediante la migración aditiva `20260903070000_add_pos_authorization_scope`. Versiona el catálogo técnico de permisos sin conceder grants, agrega `PosSession` revocable, `PosPersonalAuthorization`, asignaciones explícitas de sucursal por puesto/credencial y snapshots de presencia en `PosTicketSeller`. Ambos schemas Prisma deben conservar esta estructura sincronizada; la migración no se aplicó a development ni producción durante la implementación.
- Cada request POS online revalida la sesión persistida, la versión/actividad de la credencial, la terminal, la sucursal, los permisos vigentes y `authorizedBranchIds`. Master recibe exactamente las sucursales activas; un puesto o credencial con asignaciones recibe sólo su unión explícita —y no puede iniciar sesión en una terminal fuera de ella—; sin asignaciones, el operador conserva la sucursal de sesión. Listados, agregados y exports materializan los IDs autorizados antes de consultar; nunca interpretan `all_authorized` como ausencia de filtro.
- `POST /api/pos/personal-authorizations` sólo verifica el PIN de la credencial de la sesión y liga el token opaco a identidad, terminal, propósito y sesión por dos minutos; no acepta IDs de vendedor enviados por el renderer. Las autorizaciones master también quedan ligadas a la sesión que las solicita. `POST /api/pos/session/exit` revoca únicamente esa sesión y sus autorizaciones temporales, registra `POS_SESSION_EXIT_WITHOUT_CLOSE_DAY` y no modifica jornada, asistencia, conteos o caja.
- Clock In/Out exige código personal: la base mantiene una sola asistencia `OPEN` por empleado, la salida valida que el PIN corresponda al registro y repetir una salida `MANUAL` devuelve el mismo resultado; `CLOSE_DAY` continúa siendo un motivo distinto. `/api/pos/sale/sellers` devuelve primero asistencias abiertas de la sucursal y al propietario vigente de cartera, habilita ausentes activos sólo con búsqueda y el ticket conserva `clockedInSnapshot`, sucursal y asistencia. El backend vuelve a validar la elegibilidad del vendedor para impedir IDs forzados.
- El renderer actualiza permisos y sucursales cada 15 segundos mientras opera online. Retirar un destino cierra la vista y los diálogos protegidos mediante el guard existente; revocar sesión, credencial, terminal o sucursal devuelve al login. La operación y los historiales ya persistidos no se borran. La guía de provisión y comportamiento está en `docs/POS_AUTHORIZATION_SCOPE.md`.
...
- Las ventas, gastos, altas de catálogo, movimientos de inventario/bodega, cierres y Clock In crean notificaciones y destinatarios en el mismo commit del evento. `/api/pos/notifications` pagina por sucursales autorizadas y por credencial o permiso vigente, registra `deliveredAt`, lectura individual/masiva y preferencias administrables. Ningún mensaje incluye costos.
- `backend/api/src/routes/pos-report.routes.ts` publica `/api/pos/reports/:key` y `/api/pos/exports/:key` para ventas, caja, productos, mercancía, empleados y clientes. Los datasets exigen `REPORTS_VIEW`; exports exigen además `REPORTS_PRINT`. Ambos aplican rango, filtros, paginación y la unión exacta de sucursales autorizadas. Costo, utilidad, margen y valor de inventario se redactan salvo para master o `REPORTS_COSTS`.
- En modo API, Dashboard y X-Report usan los agregados del servidor; Receipts, Reports y los historiales operativos recorren las páginas autorizadas. Las exportaciones ejecutivas solicitan primero un dataset autorizado y mantienen XLSX/PDF como renderizado local. Los estados operativos arrancan vacíos y se hidratan desde API o desde la caché offline cifrada; `localStorage` y los datos de `mock-data.ts` quedan aislados al rollback explícito `VITE_POS_DATA_MODE=mock` hasta el piloto.
- El cierre local de la Fase 7 verificó ambos schemas Prisma sincronizados y válidos, type-check de `@cosmetics/types`, API client, API y POS, lint y build del API, build Vite de renderer/main/preload y 45 pruebas unitarias. No se ejecutaron migraciones ni pruebas HTTP contra PostgreSQL porque la validación con una base desechable/development autorizada pertenece a la Fase 8.
...
- En modo API consumen backend login, sesión, sucursales, permisos, `Employees`, catálogo/clientes/activos, inventario/bodega, tickets/checkout/apartados/adeudos/citas/vouchers, jornada, asistencia, caja, notificaciones, preferencias, Dashboard, X-Report, reportes y datasets de exportación. `VITE_POS_DATA_MODE=mock` conserva la demo únicamente como rollback para el piloto y su antigua cola `localStorage` queda aislada a ese modo; ningún dato demostrativo se migra a una BD operativa.
...
pnpm test:ui:visual
pnpm pos:visual:manifest # valida cobertura RV0 sin abrir navegador
pnpm pos:visual:capture  # requiere POS_VISUAL_MODE, POS_VISUAL_ROOT y POS_VISUAL_OUTPUT
pnpm pos:visual:capture:reference # detecta la extracción temporal y captura el baseline canónico
pnpm pos:visual:compare  # compara reference/candidate; tolerancia cero por default
pnpm test:unit
pnpm ci:build
```
...
POS_DEMO_CONFIRMATION=LOCAL_SYNTHETIC_ONLY POS_DEMO_EXPECTED_SHA=$(git rev-parse HEAD) backend/api/scripts/prepare-pos-local-demo.sh up # API/BD POS locales en loopback
backend/api/scripts/prepare-pos-local-demo.sh verify # SHA, readiness, CORS, bindings y 46 migraciones
backend/api/scripts/prepare-pos-local-demo.sh down # detiene sin eliminar la BD ni secretos locales

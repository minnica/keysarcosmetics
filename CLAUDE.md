...
- Los helpers raíz creados por una tarea automática deben ser exclusivamente POS y llamarse `scripts/{verify,prepare,provision,audit,check,seed}-pos-*` con extensión `.sh`, `.mjs` o `.ts`; el runner, sus pruebas y los scripts generales permanecen protegidos. Los helpers exclusivos del API deben preferir `backend/api/scripts/`.

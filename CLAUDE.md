...
- Salvo login, sesión, sucursales, permisos, `Employees` y las lecturas de la Fase 2 (catálogo, proveedores, vouchers y configuración de impresión), los demás módulos de `apps/pos` continúan como mocks operativos de React. No escriben todavía tickets, inventario, citas ni jornadas en backend. `VITE_POS_DATA_MODE=mock` conserva temporalmente la demo para comparación; esos datos nunca se migran a una BD operativa. La única persistencia mock sigue siendo la cola frontend de tickets offline en `localStorage`, que será sustituida en la Fase 6.

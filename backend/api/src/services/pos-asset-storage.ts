...
function storageClient() {
  ...
  if (!url || !serviceRoleKey)
    throw new Error("El almacenamiento POS no está configurado");
...
export function validateCatalogImage(file: Express.Multer.File): void {
  ...
  if (file.size <= 0 || file.size > POS_MAX_ASSET_BYTES)
    throw new Error("La imagen debe pesar entre 1 byte y 5 MB");
}

async function uploadImage(storagePrefix: string, file: Express.Multer.File) {
  validateCatalogImage(file);
  const extension =
    path.extname(file.originalname).toLocaleLowerCase("en-US") || ".img";
  const storagePath = `${storagePrefix}/${crypto.randomUUID()}${extension}`;
  const client = storageClient();
  const { error } = await client.storage
    .from(POS_ASSET_BUCKET)
...
export const uploadCatalogImage = (itemId: string, file: Express.Multer.File) =>
  uploadImage(`catalog/${itemId}`, file);

export const uploadTicketLogo = (branchId: string, file: Express.Multer.File) =>
  uploadImage(`ticket/${branchId}`, file);

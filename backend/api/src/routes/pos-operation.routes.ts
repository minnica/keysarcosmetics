...
const reportsAllowed = (req: Request, res: Response, next: NextFunction) => {
  if (
    ...
    req.posUser?.permissions.some((permission) =>
      ["DASHBOARD_VIEW", "REPORTS_VIEW", "BUSINESS_DAY_CLOSE"].includes(
        permission,
      ),
    )
  )
...
async function openDayForBranch(
  ...
  if (!day || day.status !== "OPEN") {
    throw new PosOperationError(
      "La sucursal no tiene una jornada abierta para la fecha actual",
      409,
    );
  }
  return day;
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    res.json({
      success: true,
      message: "OK",
      data: day ? businessDayDto(day) : null,
    });
  }),
);
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          ...
          if (existing)
            throw new PosOperationError(
              "La jornada de esta sucursal y fecha ya existe",
              409,
            );
          ...
          await tx.auditLog.create({
            data: {
              action: parsed.data.skipped
                ? "POS_BUSINESS_DAY_OPEN_SKIP"
                : "POS_BUSINESS_DAY_OPEN",
              outcome: "SUCCESS",
              actorCredentialId: req.posUser!.credentialId,
              terminalId: req.posUser!.terminalId,
              branchId: req.posUser!.branchId,
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          ...
          if (!day || day.branchId !== req.posUser!.branchId)
            throw new PosOperationError("Jornada no encontrada", 404);
          if (
            day.status !== "OPEN" ||
            day.closingCountId ||
            day.closingSkipped
          ...
          const updated = await tx.posBusinessDay.update({
            ...
            data: {
              closingCountId,
              closingSkipped: parsed.data.skipped,
              closingAuthorizationId,
            },
            include: businessDayInclude,
          });
          return {
            status: 201,
            message: "Conteo final registrado",
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          ...
          if (!day || day.branchId !== req.posUser!.branchId)
            throw new PosOperationError("Jornada no encontrada", 404);
          if (day.status !== "OPEN")
            throw new PosOperationError("La jornada ya fue cerrada", 409);
          ...
          const collectedTotal = tickets.reduce(
            (sum, ticket) => sum.plus(ticket.amountPaid),
            decimal(0),
          );
          const discountTotal = tickets.reduce(
            (sum, ticket) => sum.plus(ticket.discountTotal),
            decimal(0),
          );
          const expenseTotal = cashMovements.reduce(
          ...
          return {
            status: 200,
            message: "Jornada cerrada de forma inmutable",
            data: businessDayDto(updated),
          };
        },
      }),
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    const parsed = posOperationQuerySchema
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: `ATTENDANCE_CLOCK_IN:${parsed.data.branchId ?? req.posUser!.branchId}`,
        payload: {
          fingerprint: fingerprintSecret(parsed.data.pin, "pin"),
          branchId: parsed.data.branchId ?? req.posUser!.branchId,
        },
        execute: async (tx) => {
          ...
          const branchId = parsed.data.branchId ?? req.posUser!.branchId;
          ...
          const day = await openDayForBranch(tx, branchId, date);
          ...
          if (!attendance)
            throw new PosOperationError(
              "La credencial no pertenece a un vendedor",
              409,
            );
          await enqueuePosNotification(tx, {
            kind: "CLOCK_IN",
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          const [attendance, credential] = await Promise.all([
            tx.posAttendance.findUnique({
              where: { id: req.params["id"]! },
              include: attendanceInclude,
            }),
            tx.posCredential.findUnique({
              where: {
                pinFingerprint: fingerprintSecret(parsed.data.pin, "pin"),
              },
          ...
          if (
            !attendance ||
            !req.posUser!.authorizedBranchIds.includes(attendance.branchId)
          )
            throw new PosOperationError("Asistencia no encontrada", 404);
          const matches = await verifyPosSecret(
            parsed.data.pin,
            credential?.pinHash ?? POS_DUMMY_BCRYPT_HASH,
          );
          if (
            !credential?.active ||
            !credential.employee?.activo ||
            credential.employee.id !== attendance.employeeId ||
            !matches
          ) {
            throw new PosOperationError(
              "El código personal no corresponde a esta asistencia",
              403,
            );
          }
          if (attendance.status !== "OPEN") {
            if (
              attendance.closeReason === "MANUAL" &&
              attendance.closedByCredentialId === credential.id
            ) {
              return {
                status: 200,
            ...
            throw new PosOperationError(
              "La asistencia fue cerrada por la jornada",
              409,
            );
          }
          const updated = await tx.posAttendance.update({
            ...
            data: {
              ...
              clockOutAt: new Date(),
              closeReason: "MANUAL",
              closedByCredentialId: credential.id,
            },
            include: attendanceInclude,
          });
          await tx.auditLog.create({
            data: {
              action: "POS_ATTENDANCE_CLOCK_OUT",
              outcome: "SUCCESS",
              actorCredentialId: credential.id,
              terminalId: req.posUser!.terminalId,
              branchId: attendance.branchId,
              targetType: "PosAttendance",
              targetId: attendance.id,
              metadata: { closeReason: "MANUAL" },
            },
          });
          return {
            status: 200,
            message: "Salida registrada",
            data: attendanceDto(updated),
          };
        },
      }),
    );
...
router.get(
  "/expense-types",
  requireAnyPosPermission("CASH_VIEW", "CASH_MANAGE"),
  asyncRoute(async (_req, res) => {
    const types = await prisma.posExpenseType.findMany({
...
router.delete(
  ...
  requirePosPermission("SETTINGS_MANAGE"),
  asyncRoute(async (req, res) => {
    const used = await prisma.posCashExpense.count({
      where: { expenseTypeId: req.params["id"]! },
    });
    const type = await prisma.posExpenseType.update({
      where: { id: req.params["id"]! },
    ...
    res.json({
      success: true,
      message: used
        ? "Tipo inactivado; el historial se conservó"
        : "Tipo retirado",
      data: { id: type.id },
    });
  }),
  ...
  asyncRoute(async (req, res) => {
    ...
    const where: Prisma.PosCashExpenseWhereInput = {
      ...
      businessDate: new Date(
        `${parsed.data.businessDate ?? currentBusinessDate()}T00:00:00.000Z`,
      ),
    ...
    res.json({
      ...
      message: "OK",
      data: {
        items: items.map(cashExpenseDto),
        page: parsed.data.page,
        pageSize: parsed.data.pageSize,
        total,
      },
...
router.post(
  "/expenses",
  requirePosPermission("CASH_MANAGE"),
  asyncRoute(async (req, res) => {
    const key = idempotencyKey(req, res);
    if (!key) return;
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          ...
          const type = await tx.posExpenseType.findFirst({
            where: {
          ...
          if (!type)
            throw new PosOperationError("Tipo de gasto inactivo o inexistente");
          ...
          if (
            ...
            throw new PosOperationError(
              "No puedes registrar gastos para otro empleado",
              403,
            );
          ...
          const expense = await tx.posCashExpense.create({
            data: {
              ...
              employeeNameSnapshot:
                employee?.nombreCompleto ?? req.posUser!.displayName,
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        payload: parsed.data,
        execute: async (tx) => {
          const original = await tx.posCashExpense.findUnique({
            where: { id: req.params["id"]! },
          });
          if (!original || original.branchId !== req.posUser!.branchId)
            throw new PosOperationError("Gasto no encontrado", 404);
          if (original.status !== "ACTIVE")
            throw new PosOperationError(
              "El gasto ya fue anulado o corregido",
              409,
            );
          const authorization = await consumeOperationAuthorization(tx, {
            token: parsed.data.authorizationToken,
          ...
          const date = currentBusinessDate();
          const currentDay = await openDayForBranch(
            tx,
            original.branchId,
            date,
          );
          const type = await tx.posExpenseType.findFirst({
            where: { id: parsed.data.expenseTypeId, deletedAt: null },
          });
          if (!type) throw new PosOperationError("Tipo de gasto inexistente");
          const employeeId = parsed.data.employeeId ?? original.employeeId;
          const employee = employeeId
            ? await tx.empleado.findUnique({ where: { id: employeeId } })
            : null;
          await tx.posCashExpense.update({
            where: { id: original.id },
            data: {
              status: "VOIDED",
              voidedAt: new Date(),
              voidedByCredentialId: req.posUser!.credentialId,
              voidAuthorizationId: authorization.id,
            },
          });
          await tx.posCashMovement.create({
            data: {
              ...
              snapshot: expenseSnapshot(original),
            },
          });
          const replacement = await tx.posCashExpense.create({
            data: {
              ...
              employeeId,
              employeeNameSnapshot:
                employee?.nombreCompleto ?? original.employeeNameSnapshot,
              expenseTypeId: type.id,
              expenseTypeSnapshot: type.name,
              amount: decimal(parsed.data.amount),
              concept: parsed.data.concept,
          ...
          await tx.posCashMovement.create({
            data: {
              ...
              authorizationId: authorization.id,
              reason: parsed.data.reason,
              snapshot: expenseSnapshot(replacement),
            },
          });
          return {
            status: 200,
            message:
              original.businessDate.getTime() ===
              currentDay.businessDate.getTime()
                ? "Gasto corregido con trazabilidad"
                : "Corrección aplicada en la jornada actual sin reabrir el histórico",
            data: cashExpenseDto(replacement),
          };
        },
      }),
    );
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        payload: parsed.data,
        execute: async (tx) => {
          const expense = await tx.posCashExpense.findUnique({
            where: { id: req.params["id"]! },
            include: cashExpenseInclude,
          });
          if (!expense || expense.branchId !== req.posUser!.branchId)
            throw new PosOperationError("Gasto no encontrado", 404);
          if (expense.status !== "ACTIVE")
            throw new PosOperationError("El gasto ya fue anulado", 409);
          const authorization = await consumeOperationAuthorization(tx, {
            token: parsed.data.authorizationToken,
          ...
          const updated = await tx.posCashExpense.update({
            where: { id: expense.id },
            data: {
              status: "VOIDED",
              voidedAt: new Date(),
          ...
          return {
            status: 200,
            message:
              expense.businessDate.getTime() ===
              currentDay.businessDate.getTime()
                ? "Gasto anulado"
                : "Anulación compensada en la jornada actual",
            data: cashExpenseDto(updated),
          };
        },
      }),
    );
...
async function operationalSummary(req: Request) {
  ...
  const signedOperation = (kind: string) =>
    ["REFUND", "CANCELED"].includes(kind) ? -1 : 1;
  const collectedTotal = operations.reduce(
    (sum, operation) =>
      sum.plus(operation.amount.times(signedOperation(operation.kind))),
    decimal(0),
  );
  const expenseTotal = cashMovements.reduce(
    (sum, movement) => sum.plus(movement.amount),
  ...
  for (const ticket of tickets) {
    for (const seller of ticket.sellers) {
      const current = sellerMap.get(seller.employeeId) ?? {
  ...
  for (const operation of operations)
    for (const payment of operation.payments) {
      ...
      current.amount = current.amount.plus(
        payment.amount.times(signedOperation(operation.kind)),
      );
      paymentMap.set(payment.paymentMethodId, current);
    }
  const singleDay = branchId ? (days[0] ?? null) : null;
  const status = days.some((day) => day.status === "OPEN")
    ? "OPEN"
    : days.length
      ? "CLOSED"
      : "NOT_OPENED";
  return {
    ...
    unitsSold: [...productMap.values()]
      .reduce((sum, product) => sum.plus(product.quantity), decimal(0))
      .toFixed(2),
    ...
    products: [...productMap.values()]
      ...
      .sort((a, b) => Number(b.amount) - Number(a.amount)),
    ...(singleDay && auditAllowed(req)
      ? {
          inventoryAudit: {
            opening: singleDay.openingCount
              ? inventoryCountDto(
                  singleDay.openingCount,
                  true,
                  costsAllowed(req),
                )
              : null,
            closing: singleDay.closingCount
              ? inventoryCountDto(
                  singleDay.closingCount,
                  true,
                  costsAllowed(req),
                )
              : null,
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    res.json({
      success: true,
      message: "OK",
      data: await operationalSummary(req),
    });
  }),
);
...
router.get(
  "/reports/x-report",
  requireAnyPosPermission("X_REPORT_VIEW", "REPORTS_VIEW"),
  asyncRoute(async (req, res) => {
    res.json({
      success: true,
      message: "OK",

...
const warehouseAllowed = (req: Request) =>
  Boolean(
    req.posUser?.isMaster ||
    req.posUser?.permissions.includes("WAREHOUSE_MANAGE"),
  );
  ...
  if (
    ...
    req.posUser?.permissions.some(
      (permission) =>
        ...
        permission === "WAREHOUSE_BRANCH_REQUEST",
    )
  )
    return next();
  return res
...
const requireWarehouseReadAccess = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  if (
    ...
    req.posUser?.permissions.some(
      (permission) =>
        ...
        permission === "WAREHOUSE_BRANCH_VIEW",
    )
  )
    return next();
  return res
...
async function validateAdjustmentReferences(
  req: Request,
  lines: z.infer<typeof posInventoryAdjustmentBatchWriteSchema>["lines"],
) {
  ...
  if (
    ...
    throw new PosInventoryError(
      "El lote contiene productos inactivos o inexistentes",
    );
  }
  const locationIds = [
    ...new Set(
      lines
        .flatMap((line) => [line.fromLocationId, line.toLocationId])
  ...
  if (locations.length !== locationIds.length)
    throw new PosInventoryError("El lote contiene ubicaciones inválidas");
  if (
    locations.some(
      (location) =>
        location.branchId &&
    ...
    throw new PosInventoryError(
      "El lote contiene ubicaciones fuera del alcance de la terminal",
      403,
    );
  }
}

const adjustmentBatchInclude = {
  lines: true,
  movement: { select: { id: true } },
} as const;
const adjustmentBatchDto = (
  batch: Prisma.InventoryAdjustmentBatchGetPayload<{
    include: typeof adjustmentBatchInclude;
  }>,
) => ({
  id: batch.id,
  folio: batch.folio,
...
async function applyAdjustmentBatch(
  ...
  if (batch.status !== "PENDING")
    throw new PosInventoryError("El lote ya no está pendiente", 409);
  const items = await tx.catalogItem.findMany({
    where: { id: { in: batch.lines.map((line) => line.itemId) } },
  ...
  return tx.inventoryAdjustmentBatch
    .update({
      where: { id: batch.id },
      data: {
        status: "APPLIED",
...
async function reverseBatch(
  ...
  const batch = await tx.inventoryAdjustmentBatch.findUnique({
    where: { id: batchId },
    include: { movement: { include: movementInclude } },
  });
  if (!batch) throw new PosInventoryError("Lote no encontrado", 404);
  if (batch.status === "PENDING") {
  ...
  if (batch.status !== "APPLIED" || !batch.movement)
    throw new PosInventoryError("El lote ya no puede cancelarse", 409);
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    const parsed = z
      .object({ locationId: z.string().trim().min(1).optional() })
      .strict()
      .safeParse(req.query);
    if (!parsed.success)
    ...
    if (parsed.data.locationId && locations.length === 0)
      return res.status(403).json({
        success: false,
        message: "Ubicación fuera de alcance",
        data: null,
      });
  ...
  asyncRoute(async (req, res) => {
    ...
    const where: Prisma.InventoryMovementWhereInput = {
      ...(parsed.data.businessDate
        ? { businessDate: businessDateValue(parsed.data.businessDate) }
        : {}),
      lines: {
        some: {
          OR: [
    ...
    res.json({
      ...
      data: {
        items: items.map((item) =>
          inventoryMovementDto(item, costsAllowed(req)),
        ),
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const authorizedLocations = await db.inventoryLocation.findMany({
      where: {
        OR: [
          { branchId: null },
          { branchId: { in: req.posUser!.authorizedBranchIds } },
    ...
    const batches = await db.inventoryAdjustmentBatch.findMany({
      where: {
        lines: {
          some: {
            OR: [
    ...
    res.json({
      success: true,
      message: "OK",
      data: batches.map(adjustmentBatchDto),
    });
  }),
);
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: "INVENTORY_BATCH_CREATE",
        payload: parsed.data,
        execute: async (tx) => {
          const batch = await tx.inventoryAdjustmentBatch.create({
            data: {
              folio: await nextPosFolio(
                tx,
          ...
          return {
            status: 201,
            message: "Lote pendiente; el inventario aún no cambió",
            data: adjustmentBatchDto(batch),
          };
        },
      }),
    );
...
router.put(
  "/inventory/adjustment-batches/:id",
  requirePosPermission("INVENTORY_ADJUST"),
  asyncRoute(async (req, res) => {
    const key = idempotencyKey(req, res);
    if (!key) return;
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: `INVENTORY_BATCH_UPDATE:${req.params["id"]}`,
        payload: parsed.data,
        execute: async (tx) => {
          const current = await tx.inventoryAdjustmentBatch.findUnique({
            where: { id: req.params["id"]! },
          });
          if (!current) throw new PosInventoryError("Lote no encontrado", 404);
          if (current.status !== "PENDING")
            throw new PosInventoryError("El lote ya no admite edición", 409);
          const batch = await tx.inventoryAdjustmentBatch.update({
            where: { id: current.id },
            data: {
              notes: parsed.data.notes,
              lines: {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        ...
        execute: async (tx) => {
          ...
          return {
            status: 200,
            message: "Lote aplicado atómicamente",
            data: adjustmentBatchDto(updated),
          };
        },
      }),
    );
  ...
  asyncRoute(async (req, res) => {
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: `INVENTORY_BATCH_CANCEL:${req.params["id"]}`,
        payload: { id: req.params["id"] },
        execute: async (tx) => {
          ...
          return {
            ...
            message:
              batch.status === "CANCELED"
                ? "Lote cancelado sin impacto"
                : "Lote revertido mediante movimiento compensatorio",
            data: adjustmentBatchDto(batch),
          };
        },
      }),
...
router.get(
  "/inventory/counts",
  requirePosPermission("INVENTORY_VIEW"),
  asyncRoute(async (req, res) => {
    const parsed = z
      .object({
        locationId: z.string().trim().min(1),
        businessDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
        kind: z.enum(["OPENING", "CLOSING"]).optional(),
      })
      .strict()
      .safeParse(req.query);
    if (!parsed.success)
      return res.status(400).json({
        success: false,
        message: "Consulta de conteos inválida",
        data: parsed.error.flatten().fieldErrors,
      });
    await findLocationForScope(req, parsed.data.locationId);
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    await findLocationForScope(req, parsed.data.locationId);
    if (
      new Set(parsed.data.lines.map((line) => line.itemId)).size !==
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: "INVENTORY_COUNT_CREATE",
        payload: parsed.data,
        execute: async (tx) => {
          const items = await tx.catalogItem.findMany({
            where: {
          ...
          if (items.length !== parsed.data.lines.length)
            throw new PosInventoryError(
              "El conteo contiene productos inválidos",
            );
          const itemCosts = new Map(
            items.map((item) => [item.id, item.unitCost]),
          );
          const balances = await tx.inventoryBalance.findMany({
            where: {
              locationId: parsed.data.locationId,
              itemId: { in: parsed.data.lines.map((line) => line.itemId) },
            },
          });
            ...
            data: {
              ...
              lines: {
                create: parsed.data.lines.map((line) => {
                  ...
                  const difference = counted.minus(expectedQuantity);
                  return {
                    itemId: line.itemId,
                    ...
                    matchesExpected: difference.isZero(),
                  };
                }),
              },
            },
          ...
          const differences = count.lines.filter(
            (line) => !line.matchesExpected,
          );
          if (differences.length)
            await createInventoryLedgerMovement(tx, {
              type: "COUNT_ADJUSTMENT",
              reason: `${parsed.data.kind}_COUNT`,
              notes: parsed.data.notes ?? null,
          ...
          return {
            status: 201,
            message:
              "Conteo registrado; las diferencias se compensaron en el ledger",
            data: {
              ...base,
              ...(showAudit ? { notes: count.notes } : {}),
              lines,
...
async function hydratedWarehouseRequest(
  id: string,
...
async function warehouseNotification(
  tx: Prisma.TransactionClient,
  input: {
    kind:
      | "WAREHOUSE_REQUESTED"
    ...
    audiencePermission: "WAREHOUSE_MANAGE" | "WAREHOUSE_BRANCH_REQUEST";
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const where: Prisma.WarehouseRequestWhereInput = {
      OR: [
        { branchId: null },
        { branchId: { in: req.posUser!.authorizedBranchIds } },
      ],
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (input.source === "SUPPLIER" && !warehouseAllowed(req))
      return res.status(403).json({
        success: false,
        message: "El resurtido requiere administración de bodega",
        data: null,
      });
    if (
      input.source === "BRANCH" &&
    ...
    await respondIdempotent(
      ...
      executePosIdempotent({
        key,
        actorCredentialId: req.posUser!.credentialId,
        operation: "WAREHOUSE_REQUEST_CREATE",
        payload: input,
        execute: async (tx) => {
          const matrix = await tx.inventoryLocation.findFirst({
            where: { type: "WAREHOUSE", active: true },
          });
          const branchLocation = input.branchId
          ...
          if (
            input.supplierId &&
            !(await tx.posSupplier.findFirst({
          ...
          if (
            ...
            catalogItems.some((item) => !expectedKinds.includes(item.kind)) ||
            (input.source === "SUPPLIER" &&
              catalogItems.some((item) => item.supplierId !== input.supplierId))
          )
            throw new PosInventoryError(
              "La solicitud contiene artículos inválidos para su tipo o proveedor",
            );
          const priceList = input.priceListId
            ? await tx.posPriceList.findFirst({
                where: {
          ...
          const request = await tx.warehouseRequest.create({
            data: {
              folio: await nextPosFolio(
                tx,
                ...
                input.source === "SUPPLIER"
                  ? "ALM-RES"
                  : input.requestType === "PRODUCT"
                    ? "ALM-PRO"
                    : input.requestType === "TESTER"
                      ? "ALM-TST"
                      : "ALM-INS",
              ),
              source: input.source,
          ...
          await warehouseNotification(tx, {
            kind: "WAREHOUSE_REQUESTED",
            requestId: request.id,
            folio: request.folio,
            message: "Nueva solicitud pendiente",
            branchId: null,
            audiencePermission: "WAREHOUSE_MANAGE",
            actorCredentialId: req.posUser!.credentialId,
          });
          return {
            status: 201,
            message: "Solicitud creada; no se modificaron existencias",
            data: warehouseRequestDto(
              await hydratedWarehouseRequest(request.id, tx),
              costsAllowed(req),
            ),
          };
        },
...
async function warehouseAction(
  ...
  if (
    ...
    return res.status(403).json({
      success: false,
      message: "La acción requiere administración de bodega",
      data: null,
    });
  await respondIdempotent(
    res,
    executePosIdempotent({
      ...
      execute: async (tx) => {
        ...
        if (
          request.branchId &&
          !req.posUser!.authorizedBranchIds.includes(request.branchId)
        )
          throw new PosInventoryError(
            "Solicitud fuera del alcance de la terminal",
            403,
          );
        ...
        if (action === "approve-creation") {
          if (request.status !== "REQUESTED")
            throw new PosInventoryError(
              "La solicitud no espera primera aprobación",
              409,
            );
          toStatus = "CREATION_APPROVED";
          notificationKind = "WAREHOUSE_CREATION_APPROVED";
          ...
          if (request.status !== "CREATION_APPROVED")
            throw new PosInventoryError(
          ...
          if (request.source === "BRANCH") {
            const items = await tx.catalogItem.findMany({
              where: { id: { in: request.lines.map((line) => line.itemId) } },
              select: { id: true, unitCost: true },
            });
            const itemCosts = new Map(
              items.map((item) => [item.id, item.unitCost]),
            ...
            await createInventoryLedgerMovement(tx, {
              ...
              lines: request.lines.map((line) => ({
                ...
                quantity: line.quantity,
                unitCostSnapshot:
                  itemCosts.get(line.itemId) ?? line.unitCostSnapshot,
                requireSourceStock: true,
              })),
            });
          }
          toStatus = "SHIPPED";
          notificationKind = "WAREHOUSE_SHIPPED";
          notificationMessage =
            request.source === "SUPPLIER"
              ? "Resurtido autorizado y enviado al proveedor"
          ...
          if (request.status !== "SHIPPED")
            throw new PosInventoryError("La solicitud no está enviada", 409);
          const addsInventory =
            request.source === "SUPPLIER" || request.requestType === "PRODUCT";
          if (addsInventory)
            await createInventoryLedgerMovement(tx, {
              type:
                request.source === "SUPPLIER"
          ...
          toStatus = "RECEIVED";
          notificationKind = "WAREHOUSE_RECEIVED";
          notificationMessage = addsInventory
            ? "Recepción aplicada al inventario"
          ...
          if (!["SEND_APPROVED", "SHIPPED"].includes(request.status))
            throw new PosInventoryError(
              "La solicitud no puede regresar a pedidos",
              409,
            );
          if (request.status === "SHIPPED" && request.source === "BRANCH") {
            ...
            if (!original)
              throw new PosInventoryError(
                "No se encontró el envío para revertir",
                409,
              );
            await createInventoryLedgerMovement(tx, {
              type: "REVERSAL",
          ...
          toStatus = "REQUESTED";
          notificationKind = "WAREHOUSE_RETURNED";
          notificationMessage =
            "Envío regresado a pedidos; existencias restauradas";
          update = {
            status: toStatus,
            creationApprovedByCredentialId: null,
            creationApprovedAt: null,
            sendApprovedByCredentialId: null,
            sendApprovedAt: null,
            shippedAt: null,
          };
        } else {
          if (!["REQUESTED", "CREATION_APPROVED"].includes(request.status))
            throw new PosInventoryError(
              "La solicitud ya no puede cancelarse; use retorno",
              409,
            );
          toStatus = "CANCELED";
          notificationKind = "WAREHOUSE_CANCELED";
...
router.post(
  "/warehouse/requests/:id/approve-creation",
  requirePosPermission("WAREHOUSE_MANAGE"),
  asyncRoute((req, res) => warehouseAction(req, res, "approve-creation")),
);
router.post(
  "/warehouse/requests/:id/approve-send",
  requirePosPermission("WAREHOUSE_MANAGE"),
  asyncRoute((req, res) => warehouseAction(req, res, "approve-send")),
);
router.post(
  "/warehouse/requests/:id/receive",
  requireWarehouseAccess,
  asyncRoute((req, res) => warehouseAction(req, res, "receive")),
);
router.post(
  "/warehouse/requests/:id/return-to-requested",
  requirePosPermission("WAREHOUSE_MANAGE"),
  asyncRoute((req, res) => warehouseAction(req, res, "return-to-requested")),
);
router.post(
  "/warehouse/requests/:id/cancel",
  requireWarehouseAccess,
  asyncRoute((req, res) => warehouseAction(req, res, "cancel")),
);
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const allowedPermissions = req.posUser!.isMaster
      ? undefined
      : req.posUser!.permissions;
                  ...
                  ? [
                      {
                        OR: [
                          ...
                          { audiencePermission: { in: allowedPermissions } },
                        ],
                      },
                    ]
                  : []),
    ...
    const [items, total] = await Promise.all([
      db.posNotification.findMany({
        where,
        include: {
          reads: { where: { credentialId: req.posUser!.credentialId } },
          outbox: { where: { credentialId: req.posUser!.credentialId } },
        },
        orderBy: { creadoEn: "desc" },
        skip: (parsed.data.page - 1) * parsed.data.pageSize,
        take: parsed.data.pageSize,
    ...
    const outboxIds = items.flatMap((item) =>
      item.outbox.map((entry) => entry.id),
    );
    ...
    res.json({
      ...
      data: {
        items: items.map((item) => ({
          ...
          warehouseRequestId: item.warehouseRequestId,
          sourceType: item.sourceType,
          sourceId: item.sourceId,
          access: item.outbox[0]?.access ?? "VIEW",
          deliveredAt: item.outbox[0]?.deliveredAt?.toISOString() ?? null,
...
router.put(
  ...
  asyncRoute(async (req, res) => {
    const notification = await db.posNotification.findFirst({
      where: {
        id: req.params["id"]!,
        AND: [
          {
    ...
    const read = await db.posNotificationRead.upsert({
      where: {
        notificationId_credentialId: {
...
const notificationPreferencesSchema = z
  .object({
    preferences: z
      ...
      .max(POS_NOTIFICATION_KINDS.length),
  })
  .strict();

router.get(
  "/notifications/preferences",
  requireAnyPosPermission("SETTINGS_VIEW", "SETTINGS_MANAGE"),
    ...
    res.json({
      ...
      data: POS_NOTIFICATION_KINDS.map((kind) => ({
        ...
        recipients: preferences
          .filter((preference) => preference.kind === kind)
          .map((preference) => ({
            actorId:
              preference.credential.employeeId ??
              preference.credential.userId ??
              preference.credentialId,
            displayName:
              preference.credential.employee?.nombreCompleto ??
              preference.credential.user?.nombre ??
              preference.credential.alias,
...
router.put(
  ...
  asyncRoute(async (req, res) => {
    ...
    const actorIds = [
      ...new Set(
        parsed.data.preferences.flatMap((preference) =>
          preference.recipients.map((recipient) => recipient.actorId),
        ),
      ),
    ];
    const credentials =
      actorIds.length > 0
        ? await db.posCredential.findMany({
            where: {
              active: true,
              OR: [
                { id: { in: actorIds } },
    ...
    for (const credential of credentials) {
      ...
      if (credential.employeeId)
        credentialByActor.set(credential.employeeId, credential.id);
      if (credential.userId)
        credentialByActor.set(credential.userId, credential.id);
    }
    if (actorIds.some((actorId) => !credentialByActor.has(actorId))) {
      return res.status(400).json({
        success: false,
        message: "Una persona destinataria no tiene credencial POS activa",
        data: null,
      });
    }
    await db.$transaction(async (tx) => {
      await tx.posNotificationPreference.updateMany({
        data: { active: false },
      });
      for (const preference of parsed.data.preferences) {
        for (const recipient of preference.recipients) {
          ...
          await tx.posNotificationPreference.upsert({
            where: {
              kind_credentialId: { kind: preference.kind, credentialId },
            },
            create: {
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: {
            kinds: parsed.data.preferences.length,
  ...
  (error: unknown, _req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      return res.status(409).json({
        success: false,
        message: "La operación ya existe o cambió concurrentemente",
        data: null,
      });
    next(error);
  },

...
const requireCatalogRead = (req: Request, res: Response, next: NextFunction) => {
  if (req.posUser?.isMaster || req.posUser?.permissions.some((permission) => ["CATALOG_VIEW", "SALE_CREATE", "INVENTORY_VIEW", "INVENTORY_ADJUST", "WAREHOUSE_MANAGE", "WAREHOUSE_BRANCH_REQUEST"].includes(permission))) return next();
  return res.status(403).json({ success: false, message: "Permiso POS insuficiente", data: null });
};
const requireCustomerReadOrSale = (req: Request, res: Response, next: NextFunction) => {
  if (req.posUser?.isMaster || req.posUser?.permissions.some((permission) => permission === "CUSTOMERS_VIEW" || permission === "SALE_CREATE")) return next();
  return res.status(403).json({ success: false, message: "Permiso POS insuficiente", data: null });
};
const requirePaymentReadOrSale = (req: Request, res: Response, next: NextFunction) => {
  if (req.posUser?.isMaster || req.posUser?.permissions.some((permission) => permission === "PAYMENTS_MANAGE" || permission === "SALE_CREATE")) return next();
  return res.status(403).json({ success: false, message: "Permiso POS insuficiente", data: null });
};
const requirePackageReadOrSale = (req: Request, res: Response, next: NextFunction) => {
  if (req.posUser?.isMaster || req.posUser?.permissions.some((permission) => permission === "CATALOG_VIEW" || permission === "SALE_CREATE")) return next();
  return res.status(403).json({ success: false, message: "Permiso POS insuficiente", data: null });
};
...
router.get("/customers/sources", requireCustomerReadOrSale, async (_req, res) => { const items = await db.customerSource.findMany({ where: { deletedAt: null }, orderBy: { name: "asc" } }); res.json({ success: true, message: "OK", data: items.map(({ id, name, active }) => ({ id, name, active })) }); });

...
const access = {
  ...
  lecturaMetodosPago: requireAnyScreenAccess([
    "metodos-pago",
  ...
  .superRefine((data, ctx) => {
    if (data.tipoCompra === null && data.montoCompra !== 0) {
      ctx.addIssue({
        ...
        path: ["montoCompra"],
        message: "Una cita sin compra debe tener monto cero",
      });
    }
    if (data.tipoCompra !== null && data.montoCompra <= 0) {
      ctx.addIssue({
    ...
    if (
      data.tipoCompra === TipoCompraCita.COMPRA_CON_APARTADO &&
    ...
    if (
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["montoApartado"],
        message: "El pago de apartado solo aplica a una compra con apartado",
      });
    }
    if (
      data.estatus !== EstatusCita.ATENDIDA &&
      (data.tipoCompra !== null ||
        data.montoCompra !== 0 ||
        data.montoApartado !== 0)
    ) {
      ctx.addIssue({
    ...
    if (
      data.estatus !== EstatusCita.ATENDIDA &&
      (data.bonoSalidaTarde || data.bonoComida)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["estatus"],
        message: "Una cita no atendida no puede registrar bonos",
      });
    }
  });
...
function dateRangeSql(
  ...
  if (fechaFin) {
    conditions.push(
      Prisma.sql`v."fecha" <= ${new Date(`${fechaFin}T23:59:59`)}`,
    );
  }
...
function resolveAppointmentDateRange(
  fechaInicio?: string,
  fechaFin?: string,
...
function serializeAppointment(record: AppointmentWithRelations) {
  ...
  return {
    ...
    montoApartado,
    total:
      record.tipoCompra === TipoCompraCita.COMPRA_CON_APARTADO
        ? montoApartado
        : montoCompra,
...
async function canViewSalary(
  ...
  return Boolean(
    ...
    access?.screenPermissions.includes("empleados/sueldo"),
  );
}

async function canViewKeysarHomeData(
  req: Parameters<typeof resolveAccessForRequest>[0],
): Promise<boolean> {
  const access = await resolveAccessForRequest(req);
  return Boolean(
    access?.canManageAccess ||
    access?.screenPermissions.includes("reportes/ver-datos-keysar-home"),
  );
}
...
function selfDataCondition(employeeId: string | null): Prisma.Sql {
  return employeeId
    ? Prisma.sql`v."vendedorId" = ${employeeId}`
    : Prisma.sql`TRUE`;
}

async function canManageSale(
  req: Parameters<typeof resolveAccessForRequest>[0],
  vendedorId: string,
): Promise<boolean> {
...
router.post("/sucursales", access.sucursales, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al crear sucursal" });
  }
});
...
router.get("/empleados", access.empleados, async (req, res) => {
  try {
    ...
    const response = data.map((empleado) =>
      redactSalary(
        empleado as typeof empleado & { sueldo?: unknown },
        salaryVisible,
      ),
    );
  ...
  try {
    ...
    if (positionId) {
      const position = await prisma.position.findFirst({
        where: { id: positionId, activo: true },
      });
      if (!position) {
        res
          .status(400)
    ...
    res.status(201).json({
      success: true,
      data: redactSalary(
        data as typeof data & { sueldo?: unknown },
        salaryVisible,
      ),
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al crear empleado" });
  }
});
    ...
    if (ventaCount > 0) {
      res
        ...
        .json({
          ...
          data: null,
          message:
            'No se puede eliminar: el empleado tiene ventas registradas. Usa "Desactivar" en su lugar.',
        });
      return;
    }
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        data: null,
        message: "Error al actualizar estatus del empleado",
      });
  }
});
...
router.post("/metodos-pago", access.metodosPago, async (req, res) => {
  try {
    ...
    const data = await prisma.metodoPago.create({
      data: {
        nombre,
        tipo: tipo as "EFECTIVO" | "TARJETA" | "TRANSFERENCIA" | "OTRO",
      },
    });
    res
      .status(201)
      .json({ success: true, data, message: "Método de pago creado" });
  } catch (err) {
    console.error(err);
    res
      .status(500)
...
router.put("/metodos-pago/:id", access.metodosPago, async (req, res) => {
  try {
    const { nombre, tipo, activo } = req.body as Partial<{
      nombre: string;
      tipo: string;
      activo: boolean;
    }>;
...
router.get("/banks", access.banks, async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al obtener bancos" });
  }
});
  ...
  try {
    ...
    if (duplicate) {
      if (!duplicate.activo) {
        ...
        res
          .status(201)
          .json({ success: true, data, message: "Banco reactivado" });
        return;
      }
      res
        .status(409)
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al crear banco" });
  }
});
...
router.put("/banks/:id", access.banks, async (req, res) => {
  try {
    ...
    if (!existing) {
      res
        .status(404)
        .json({ success: false, data: null, message: "Banco no encontrado" });
      return;
    }
    const duplicate = await prisma.bank.findFirst({
      where: {
...
router.delete("/banks/:id", access.banks, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al eliminar banco" });
  }
});
  ...
  try {
    ...
    if (duplicate) {
      if (!duplicate.activo) {
        ...
        res
          .status(201)
          .json({ success: true, data, message: "Puesto reactivado" });
        return;
      }
      res
        .status(409)
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al crear puesto" });
  }
});
...
router.put("/positions/:id", access.positions, async (req, res) => {
  try {
    ...
    if (!existing) {
      res
        .status(404)
        .json({ success: false, data: null, message: "Puesto no encontrado" });
      return;
    }
    const duplicate = await prisma.position.findFirst({
      where: {
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        data: null,
        message: "Error al obtener catálogos de citas",
      });
  }
});
...
router.get("/servicios", access.servicios, async (_req, res) => {
  try {
    const data = await prisma.categoriaAtencion.findMany({
      orderBy: [{ activa: "desc" }, { nombre: "asc" }],
      include: {
        subcategorias: { orderBy: [{ activa: "desc" }, { nombre: "asc" }] },
      },
    });
    res.json({ success: true, data, message: "OK" });
  } catch (err) {
    ...
    res
      ...
      .json({
        success: false,
        data: null,
        message: "Error al obtener servicios de atención",
      });
  }
});
...
router.post("/servicios/categorias", access.servicios, async (req, res) => {
  try {
    const parsed = z
      .object({ nombre: z.string().trim().min(2).max(80) })
      .safeParse(req.body);
    if (!parsed.success) {
      res
        .status(400)
...
router.post("/servicios/subcategorias", access.servicios, async (req, res) => {
  try {
    ...
    const categoria = await prisma.categoriaAtencion.findFirst({
      where: { id: parsed.data.categoriaId, activa: true },
      select: { id: true },
    });
    if (!categoria) {
      res
    ...
    res
      .status(201)
      .json({ success: true, data, message: "Subcategoría creada" });
  } catch (err) {
    if (
      err instanceof Prisma.PrismaClientKnownRequestError &&
      err.code === "P2002"
  ...
  async (req, res) => {
    try {
      await prisma.$transaction([
        prisma.categoriaAtencion.update({
          where: { id: req.params["id"] },
          data: { activa: false },
        }),
        prisma.subcategoriaAtencion.updateMany({
...
router.delete(
  "/servicios/subcategorias/:id",
  access.servicios,
  async (req, res) => {
    try {
      await prisma.subcategoriaAtencion.update({
        where: { id: req.params["id"] },
        data: { activa: false },
      });
      res.json({
...
router.get("/citas", access.citas, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: records.map(serializeAppointment),
      message: "OK",
    });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al obtener citas" });
  }
});
  ...
  try {
    ...
    const [sucursal, vendedor, facialista, subcategoria] = await Promise.all([
      ...
      prisma.empleado.findFirst({
        where: { id: parsed.data.vendedorId, activo: true },
        select: { id: true },
      }),
      prisma.empleado.findFirst({
        where: { id: parsed.data.facialistaId, activo: true },
        select: { id: true },
      }),
      prisma.subcategoriaAtencion.findFirst({
        where: {
          id: parsed.data.subcategoriaId,
          activa: true,
    ...
    if (!sucursal || !vendedor || !facialista || !subcategoria) {
      res
        ...
        .json({
          success: false,
          data: null,
          message: "Sucursal, vendedor, facialista o servicio inválido",
        });
      return;
    }
    ...
    const record = await prisma.registroCita.create({
      data: {
        ...
        montoApartado: new Prisma.Decimal(parsed.data.montoApartado),
    ...
    res
      ...
      .json({
        success: true,
        data: serializeAppointment(record),
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al registrar cita" });
  }
});
...
router.put("/citas/:id", access.citas, async (req, res) => {
  try {
    ...
    if (!fecha) {
      res
        .status(400)
        .json({ success: false, data: null, message: "Fecha inválida" });
      return;
    }

    const [existing, sucursal, vendedor, facialista, subcategoria] =
      await Promise.all([
...
router.delete("/citas/:id", access.citas, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al eliminar cita" });
  }
});
...
router.get("/ventas", access.ventas, async (req, res) => {
  try {
    ...
    const rangeStart = startOfDay(
      parsedStart ?? addDays(rangeEnd, -(DEFAULT_VENTAS_LOOKBACK_DAYS - 1)),
    );
    ...
    if (daysBetween(rangeStart, rangeEnd) > MAX_VENTAS_RANGE_DAYS) {
      res
        ...
        .json({
          success: false,
          data: null,
          message: `El rango máximo de consulta es de ${MAX_VENTAS_RANGE_DAYS} días`,
        });
      return;
    }
    ...
    const take = requestedLimit
      ? Math.min(requestedLimit, MAX_VENTAS_LIMIT)
      : undefined;
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al obtener ventas" });
  }
});

router.post("/ventas", access.ventas, async (req, res) => {
  try {
    const { sucursalId, vendedorId, fecha, notas, sesionId, detalles } =
      req.body as {
        sucursalId: string;
        vendedorId: string;
        fecha: string;
    ...
    if (!(await canManageSale(req, vendedorId))) {
      res
        ...
        .json({
          success: false,
          data: null,
          message: "Solo puedes registrar ventas propias",
        });
      return;
    }
    ...
    const invalidSale =
      ...
      ventas.some(
        (venta) =>
          ...
          venta.detalles.some(
            (detalle) => detalle.cantidad <= 0 || !detalle.metodoPagoId,
          ),
      );
    if (invalidSale) {
      res
  ...
  try {
    ...
    if (ownEmployeeId) {
      const sale = await prisma.venta.findFirst({
        where: { id: req.params["id"], vendedorId: ownEmployeeId },
        select: { id: true },
      });
      if (!sale) {
        res
...
router.delete("/ventas/:id", access.ventas, async (req, res) => {
  try {
      ...
      if (!sale) {
        res
          ...
          .json({
            success: false,
            data: null,
            message: "Solo puedes eliminar ventas propias",
          });
        return;
      }
    }
    ...
    res
      .status(500)
      .json({ success: false, data: null, message: "Error al eliminar venta" });
  }
});
...
router.get("/reportes/citas", access.reporteCitas, async (req, res) => {
  try {
    ...
    if (sucursalId)
      conditions.push(Prisma.sql`rc."sucursalId" = ${sucursalId}`);
    ...
    >`
      SELECT
        ...
        COALESCE(SUM(CASE WHEN rc."tipoCompra" = 'COMPRA_CON_APARTADO' THEN rc."montoApartado" WHEN rc."tipoCompra" = 'PAGO_DE_APARTADO' THEN rc."montoCompra" ELSE 0 END), 0)::float AS "pagoDeApartado",
        COALESCE(SUM(CASE WHEN rc."tipoCompra" = 'PAGO_NETO' THEN rc."montoCompra" WHEN rc."tipoCompra" = 'COMPRA_CON_APARTADO' THEN rc."montoApartado" WHEN rc."tipoCompra" = 'PAGO_DE_APARTADO' THEN rc."montoCompra" ELSE 0 END), 0)::float AS "total",
...
router.get(
  "/reportes/metodo-pago-por-dia",
  access.metodoPagoPorDia,
  async (req, res) => {
    try {
      const { metodoPagoId, mes, anio } = req.query as {
        metodoPagoId?: string;
        mes?: string;
        anio?: string;
      };
...
router.get(
  ...
  access.ventasPorVendedorDia,
  async (req, res) => {
    try {
      const { vendedorId, fechaInicio, fechaFin } = req.query as {
        vendedorId?: string;
        fechaInicio?: string;
        fechaFin?: string;
      };
...
router.get("/reportes/total-general", access.totalGeneral, async (req, res) => {
  try {
    ...
    const grouped = new Map<
      string,
      Array<{ sucursalId: string; sucursalNombre: string; total: number }>
    >();
...
router.get("/reportes/dashboard", access.dashboard, async (req, res) => {
  try {
    ...
    const periodValues = Prisma.join(
      periods.map(
        (period) => Prisma.sql`(
      ${period.key},
      ${period.start},
    ...
    res.json({
      success: true,
      data: { dia, mes, anio, monthsData, ventasPorVendedor },
      message: "OK",
    });
  } catch (err) {
    console.error(err);

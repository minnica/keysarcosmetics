...
- `Local` y `Profesional` son entidades separadas; `Planes` en Comisiones muestra por ahora un estado vacío mock, sin persistencia. `Consentimientos` administra documentos, pero todavía no incluye firma ni resultados; `Encuestas` configura preguntas, pero todavía no incluye resultados.

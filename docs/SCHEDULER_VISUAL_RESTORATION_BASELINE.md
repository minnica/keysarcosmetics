...
| `/administracion?section=locals`                         | Comercios y sucursales                                                     | Catálogo/candidatos reales                         | Restaurar en RV4                                                                 |
| `/administracion?section=professionals`                  | Especialistas y grupos                                                     | Activación de empleados reales                     | Restaurar en RV4                                                                 |
| `/administracion?section=services`                       | Servicios, categorías, clases, paquetes y adicionales                      | Perfiles de servicios + editor avanzado            | Restaurar en RV4                                                                 |
| `/administracion?section=commissions`                    | Reglas y escalas                                                           | Política real simplificada                         | Restaurar en RV4                                                                 |
| `/administracion?section=resources`                      | Recursos, horarios y validación                                            | CRUD base real                                     | Restaurar en RV4                                                                 |
| `/administracion?section=surveys`                        | Encuestas y preguntas                                                      | Alta mínima de una pregunta rating                 | Restaurar en RV6                                                                 |
| `/administracion?section=consents`                       | Plantillas y documentos                                                    | Carga privada real                                 | Restaurar en RV6                                                                 |
| `/administracion?section=whatsapp`                       | Mensajes y plantillas                                                      | Plantillas/outbox reales                           | Restaurar en RV6                                                                 |
| `/administracion?section=gift-cards`                     | Plantillas de servicio/monto y diseño                                      | Alta mínima de monto                               | Restaurar en RV4                                                                 |
| `/administracion?section=status-colors`                  | Paleta editable                                                            | API real + autorización secundaria                 | Restaurar en RV4 conservando seguridad                                           |
| `/configuraciones?section=company`                       | Empresa, sitio público, LinkPro, redes y contacto                          | Editor JSON versionado                             | Restaurar formulario en RV5                                                      |
| `/configuraciones?section=website`                       | Estado pendiente de diseño                                                 | Editor JSON versionado                             | Definir campos/consumidores antes de restaurar                                   |
| `/configuraciones?section=agenda`                        | Intervalo, solapes, límites, horarios y campos                             | Editor JSON versionado                             | Restaurar formulario en RV5                                                      |
| `/configuraciones?section=payments`                      | Banco, pagos online y estado público                                       | Editor JSON versionado                             | POS conserva autoridad; revisar cada campo en RV5                                |
| `/configuraciones?section=reminders`                     | Canales para cambios y recordatorios                                       | Editor JSON versionado                             | Conectar efectos de engagement en RV5/RV6                                        |
| `/configuraciones?section=records`                       | Categorías/campos de ficha médica                                          | Editor JSON + endpoints médicos separados          | Restaurar sin mezclar documento y expediente                                     |
| `/configuraciones?section=emails`                        | Remitentes, firma y cumpleaños                                             | Editor JSON versionado                             | Restaurar y mapear proveedor/plantillas                                          |
| `/configuraciones?section=integrations`                  | Estado pendiente de diseño                                                 | Editor JSON versionado                             | No inventar integraciones; documentar claves soportadas                          |
| `/configuraciones?section=notifications`                 | Estado pendiente de diseño                                                 | Editor JSON versionado                             | No inventar consumidores                                                         |
| `/configuraciones?section=clients`                       | Duplicados, categorías, campos y filtros                                   | Editor JSON; definiciones reales tienen API propia | Usar contratos de definiciones en RV5                                            |
| `/configuraciones?section=surveys`                       | Activación y demora de envío                                               | Editor JSON versionado                             | Conectar con encuestas/outbox en RV5/RV6                                         |
...
| Clientes listado/filtros/paginación             | `ClientsDatabase`                                                                         | `searchCustomers`, sources, field definitions                                             | `clients:READ`                                                | API UI fija 50 y no pagina; restaurar filtros sin derivar sobre página parcial                          | Inventariado                    |
| Clientes alta/edición                           | `ClientsWorkspace`                                                                        | sources / create, update                                                                  | `clients:WRITE`                                               | Mapear alias, preferencia, procedencia, cartera, campos y versión                                       | Inventariado                    |
| Cliente ficha/históricos                        | diálogos de `ClientsWorkspace`                                                            | detail, visits, financial-history                                                         | `clients:READ` + autorizaciones separadas                     | API UI pide las tres a la vez; mantener tokens/errores/paginación independientes                        | Inventariado                    |
| Clientes combinar/exportar/audiencia/fichas     | diálogos de `ClientsDatabase`                                                             | search/report / merge, export                                                             | `clients:WRITE/EXPORT`; `CLIENT_MERGE`, `SENSITIVE_EXPORT`    | Merge y export existen; audiencias y reporte de fichas no tienen contrato equivalente                   | Brecha B03                      |
| Comercios/sucursales                            | `CommerceDialog`, `LocalDialog`, `SpecialDayDialog`                                       | candidates/catalog / create/update commerce, update branch, availability rules/exceptions | `administration/locals:ADMIN`                                 | UI actual no expone toda edición de comercio, identidad/contacto/imagen ni días especiales              | Brecha B03                      |
| Especialistas/grupos                            | `ProfessionalDialog`, `GroupDialog`                                                       | candidates/catalog / profile, specialty, group, service assignments, availability         | `administration/professionals:ADMIN`                          | Sólo activar personas existentes; UI actual no cubre grupos/especialidades/horario completo             | Brecha B03                      |
| Servicios/categorías/precios                    | `ServiceDialog`, `CategoryDialog`, `BulkPriceDialog`                                      | candidates/catalog/admin catalog / service, package, addon, class schedule, requirements  | `administration/services:ADMIN`                               | Categorías y archivos masivos no tienen cobertura demostrada; POS es autoridad de precio                | Brecha B03                      |
| Comisiones                                      | `CommissionDialog`                                                                        | admin catalog + POS refs / `updateCommissionPolicy`                                       | `administration/commissions:ADMIN`                            | Mapear reglas/escalas históricas; Nómina conserva pago final                                            | Inventariado                    |
| Recursos                                        | `ResourceDialog`, `ScheduledResourceDialog`, `ResourceValidationDialog`                   | catalog / resource, requirements, availability                                            | `administration/resources:ADMIN`                              | CRUD base existe; horarios, validaciones y asociaciones no están en UI actual                           | Brecha B03                      |
| Encuestas                                       | `SurveyDialog`, `SurveyQuestionDialog`                                                    | surveys / create, update, issue token                                                     | `administration/surveys:ADMIN`                                | UI actual crea una pregunta rating; restaurar preguntas/estado/servicios sin tokens visibles            | Inventariado                    |
| Consentimientos                                 | `ConsentDialog`, file dropzone                                                            | consent templates/records / upload, assign, status                                        | `administration/consents:ADMIN` + autorizaciones privadas     | Cubrir edición/versiones/firma y URLs efímeras; requiere storage para integración                       | Inventariado                    |
| WhatsApp                                        | `WhatsAppDialog`, `TemplateDialog`                                                        | templates/outbox / create, update, enqueue, retry                                         | `administration/whatsapp:ADMIN`                               | Diferenciar encolado/entrega, consentimiento y proveedor disabled                                       | Inventariado                    |
| Gift cards                                      | `GiftCardDialog`                                                                          | admin catalog + POS refs / create, update                                                 | `administration/gift-cards:ADMIN`                             | UI actual sólo alta de monto; saldo/redención no pertenecen al contrato Scheduler                       | Brecha B03                      |
| Colores                                         | `StatusColorsSection`                                                                     | admin catalog / update colors                                                             | `administration/status-colors:ADMIN` + `STATUS_COLORS_CHANGE` | Conservar control optimista y autorización segura                                                       | Inventariado                    |
| Configuración 11 documentos                     | paneles de `SettingsWorkspace`                                                            | `resolvedSetting` / `updateSetting`                                                       | pantalla de sección `READ/WRITE`                              | Mapear campo, default, alcance y consumidor; preservar claves desconocidas y borrador por contexto      | Brecha B04                      |
...
| B03 | Faltan o no están expuestas en la UI real varias acciones históricas: identidad/contacto/imagen, importación/exportación de precios, audiencias, reporte de fichas, grupos/especialidades completos, horarios de recursos, encuestas completas, edición de consentimientos/plantillas y gift cards de servicio. | Cobertura API/UI               | RV3/RV4/RV6                                     |
| B04 | Los once documentos JSON se versionan y fusionan `COMMERCE → BRANCH → USER`, pero la búsqueda de consumidores no encontró lectura operativa de esas claves. Endpoints de expedientes y campos usan permisos de settings, no los documentos.                                                                     | Efecto funcional               | RV5                                             |
...
| B08 | La referencia contiene mutaciones locales de pagos e información financiera. El backend actual declara POS como autoridad y sólo permite lectura protegida desde Scheduler.                                                                                                                                     | Diferencia semántica necesaria | RV2/RV3                                         |

...
- Si el bloque requiere un helper nuevo en `scripts/`, debe ser exclusivamente POS, usar uno de los nombres `verify-pos-*`, `prepare-pos-*`, `provision-pos-*`, `audit-pos-*`, `check-pos-*` o `seed-pos-*` y extensión `.sh`, `.mjs` o `.ts`; sus pruebas pueden insertar `.test` antes de `.mjs` o `.ts`. Prefiere `backend/api/scripts/` cuando sólo lo consume el API. El ejecutor y sus pruebas siguen fuera de alcance.
- Conserva los datos/outboxes y cualquier trabajo previo. No incluyas secretos, archivos `.env` reales, logs crudos o volcados en documentos versionados. Sólo `apps/pos/.env.example` puede actualizarse como plantilla pública con URLs loopback y marcadores no reutilizables; los demás ejemplos de entorno siguen fuera de alcance.
...
- `remaining` significa exclusivamente trabajo faltante dentro del criterio asignado: debe ser `[]` cuando uses `completed`. Las limitaciones o seguimientos pertenecientes a otros IDs, tareas diferidas, PO, Operación o ambientes externos se conservan en el informe y `nextStep`, pero no convierten en parcial un criterio local ya satisfecho. Si accidentalmente los incluyes con un check marcado `[x]` y todas las pruebas pertinentes aprobadas, el ejecutor los preservará como `followUps` y continuará.

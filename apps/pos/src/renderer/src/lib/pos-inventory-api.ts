...
const directionFor = (
  line: PosInventoryMovementDto["lines"][number],
): InventoryMovementDirection =>
...
const categoryFor = (
  type: PosInventoryMovementDto["type"],
): InventoryMovementCategory => {
...
export const inventoryMovementsFromDto = (
  items: PosInventoryMovementDto[],
): InventoryMovement[] =>
  items.flatMap((movement) =>
    movement.lines.map((line) => {
      const unitCost = Number(
        "unitCostSnapshot" in line ? (line.unitCostSnapshot ?? "0.00") : "0.00",
      );
      return {
        ...
        previousStock: Number(
          line.fromQuantityBefore ?? line.toQuantityBefore ?? "0.00",
        ),
        newStock: Number(
          line.fromQuantityAfter ?? line.toQuantityAfter ?? "0.00",
        ),
        sourceBranch:
          line.fromLocation?.branchName ??
          line.fromLocation?.name ??
          "Entrada externa",
        destinationBranch:
          line.toLocation?.branchName ?? line.toLocation?.name ?? null,
        destinationPreviousStock:
          line.toQuantityBefore === null ? null : Number(line.toQuantityBefore),
        destinationNewStock:
          line.toQuantityAfter === null ? null : Number(line.toQuantityAfter),
  ...
  return {
    ...
    adjustments: batch.lines.map((line) => ({
      ...
      direction:
        line.type === "ADD"
          ? "ADD"
          : ["TRANSFER", "RETURN"].includes(line.type)
            ? "TRANSFER"
            : "REMOVE",
      reason: line.reason ?? line.type,
      quantity: Number(line.quantity),
      sourceBranch: line.fromLocationId
        ? (names.get(line.fromLocationId) ?? "")
        : "",
      destinationBranch: line.toLocationId
        ? (names.get(line.toLocationId) ?? null)
        : null,
      comment: line.notes ?? "",
...
const warehouseStatus = (
  status: PosWarehouseRequestDto["status"],
): WarehouseMovementStatus => {
...
export const warehouseMovementFromDto = (
  request: PosWarehouseRequestDto,
): WarehouseMovement => {
  ...
  const creation = request.events.find(
    (event) =>
      event.action === "APPROVE-CREATION" ||
      event.action === "APPROVE_CREATION",
  );
  const send = request.events.find(
    (event) =>
      event.action === "APPROVE-SEND" || event.action === "APPROVE_SEND",
  );
  const receive = request.events.find((event) => event.action === "RECEIVE");
  const cancel = request.events.find((event) => event.action === "CANCEL");
  const returned = request.events.find(
    (event) =>
      event.action === "RETURN-TO-REQUESTED" ||
      event.action === "RETURN_TO_REQUESTED",
  );
  return {
    id: request.id,
    backendVersion: request.version,
    folio: request.folio,
    ...
    createdByName: request.events[0]?.actorName ?? "POS",
    creationApprovedAtIso: request.creationApprovedAt,
    creationApprovedByName: creation?.actorName ?? null,
    sentAtIso: request.shippedAt,
    sentByName: send?.actorName ?? null,
    receivedAtIso: request.receivedAt,
    receivedByName: receive?.actorName ?? null,
    cancelledAtIso: request.canceledAt,
    cancelledByName: cancel?.actorName ?? null,
    returnedToOrdersAtIso: returned?.createdAt ?? null,
    returnedToOrdersByName: returned?.actorName ?? null,
  };
};

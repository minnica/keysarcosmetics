...
export function WarehouseStockView({ products, supplies, suppliers, stock, movements, canManage, canViewCosts, onToggleVisibility, onSaveSupply, onDeleteSupply, onCreateRestockOrder }: WarehouseStockViewProps) {
  ...
  const exportInventoryPdf = async () => {
    ...
    const protectedCostHeaders = canViewCosts ? ["Costo MXN", "Costo USD", "Socio"] : [];
    autoTable(doc, { startY: 73, head: [["SKU", "Producto", "Proveedor", "Familia / categoría", "Presentación", "Stock", "Mín.", "Máx.", ...protectedCostHeaders]], body: rows.map((row) => [row.sku, row.name, row.supplierName, `${row.family} / ${row.category}`, `${row.presentation} · ${row.unitsPerPackage} pz`, row.quantity, row.stockMin, row.stockMax, ...(canViewCosts ? [formatCurrency(row.costMxn), `$${row.costUsd.toFixed(2)}`, formatCurrency(row.partnerCost)] : [])]), styles: { fontSize: 7 }, headStyles: { fillColor: [109, 82, 61] } });

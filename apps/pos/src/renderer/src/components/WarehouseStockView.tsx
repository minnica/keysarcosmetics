...
export function WarehouseStockView({
  ...
  const [restockSupplierId, setRestockSupplierId] = useState(
    suppliers.find((supplier) => supplier.active)?.id ?? "",
  );
  ...
  const entriesFor = (id: string) =>
    movements
      .filter(
        (movement) =>
          (movement.kind === "ENTRY" || movement.kind === "PURCHASE_ORDER") &&
  ...
  const outputsFor = (id: string) =>
    movements
      ...
      .filter((line) => line.productId === id)
      .reduce((sum, line) => sum + line.quantity, 0);
  const stockState = (quantity: number, minimum: number, maximum: number) =>
    quantity < minimum
      ? "is-low"
  ...
  const exportInventoryExcel = async () => {
    ...
    const sheet = XLSX.utils.json_to_sheet(
      rows.map((row) => ({
        ...
        "Piezas por caja": row.unitsPerPackage,
        Unidad: row.unit,
        Existencia: row.quantity,
        "Stock mínimo": row.stockMin,
        "Stock máximo": row.stockMax,
        Ingresos: entriesFor(row.id),
        Salidas: outputsFor(row.id),
        ...(canViewCosts
          ? {
              "Costo MXN": row.costMxn,
              "Costo USD": row.costUsd,
              "Precio socio": row.partnerCost,
            }
          : {}),
        "Precio sugerido": row.retailPrice,
      })),
    );
    ...
    XLSX.writeFile(
      book,
      `inventario-bodega-${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  };
  ...
  const exportInventoryPdf = async () => {
    ...
    doc.setFontSize(18);
    doc.text("KEYSAR · INVENTARIO GENERAL DE BODEGA", 38, 42);
    doc.setFontSize(9);
    doc.text(
      `${rows.length} artículos · ${rows.reduce((sum, row) => sum + row.quantity, 0)} unidades`,
      38,
      58,
    );
    const protectedCostHeaders = canViewCosts
      ? ["Costo MXN", "Costo USD", "Socio"]
      : [];
    autoTable(doc, {
      startY: 73,
      head: [
      ...
      body: rows.map((row) => [
        ...
        `${row.presentation} · ${row.unitsPerPackage} pz`,
        row.quantity,
        row.stockMin,
        row.stockMax,
        ...(canViewCosts
          ? [
              formatCurrency(row.costMxn),
              `$${row.costUsd.toFixed(2)}`,
              formatCurrency(row.partnerCost),
            ]
          : []),
      ]),
      styles: { fontSize: 7 },
  ...
  const proposedForSupplier = (supplierId: string) =>
    rows
      .filter(
        (row) => row.supplierId === supplierId && row.stockMax > row.quantity,
      )
      .map((row) => ({ id: row.id, quantity: row.stockMax - row.quantity }));
  const openRestock = () => {
    const supplierId =
      suppliers.find(
        (supplier) =>
          supplier.active && proposedForSupplier(supplier.id).length > 0,
      )?.id ??
      suppliers.find((supplier) => supplier.active)?.id ??
      "";
    ...
    setRestockComment(
      "Pedido sugerido para completar el stock máximo de bodega.",
    );
    setRestockOpen(true);
  };
  const selectRestockSupplier = (supplierId: string) => {
    setRestockSupplierId(supplierId);
    setRestockLines(proposedForSupplier(supplierId));
  };
  const submitRestock = async () => {
      ...
      return [
        {
          ...
          sku: row.sku,
          itemType: row.source === "SUPPLY" ? "SUPPLY" : "PRODUCT",
          quantity: draft.quantity,
          unitCostUsd: row.costUsd,
          unitCostMxn: row.costMxn,
          partnerCost: row.partnerCost,
          partnerCostUsd: row.costUsd,
    ...
    if (!(await onCreateRestockOrder(restockSupplierId, lines, restockComment)))
      return;
    setRestockOpen(false);
  };

  const saveItem = async () => {
    if (itemDraft && (await onSaveSupply(itemDraft))) setItemDraft(null);
  };

  return (
    <div className="view-stack warehouse-stock-catalog">
      <Card className="warehouse-panel">
        <CardContent>
          <div className="warehouse-panel-heading">
            <div>
              <span>EXISTENCIAS MATRIZ</span>
              <h2>Inventario general de bodega</h2>
              <p>
                Productos e insumos con proveedor, presentación, costos y
                límites de resurtido.
              </p>
            </div>
            <div>
              <Button
              ...
              <Button
                variant="outline"
                onClick={() => void exportInventoryPdf()}
              >
              ...
              {canManage && (
                <>
                  <Button variant="outline" onClick={openRestock}>
                    <RefreshCw size={16} /> Generar pedido / resurtido
                  </Button>
                  <Button
                    onClick={() =>
                      setItemDraft(
                        newSupply(
                          suppliers.find((supplier) => supplier.active),
                        ),
                      )
                    }
                  >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Ingresos / salidas</TableHead>
                  {canViewCosts && (
                    <>
                      <TableHead>Costo MXN / USD</TableHead>
                      <TableHead>Precio socio</TableHead>
                    </>
                  )}
                ...
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      ...
                      <small>
                        {row.unitsPerPackage} piezas · {row.unit}
                      </small>
                    </TableCell>
                    <TableCell>
                      <strong
                        className={`warehouse-stock-number ${stockState(row.quantity, row.stockMin, row.stockMax)}`}
                      >
                        {row.quantity}
                      </strong>
                    </TableCell>
                    <TableCell>
                      ...
                      <small>
                        {Math.max(0, row.stockMax - row.quantity)} para máximo
                      </small>
                    </TableCell>
                    <TableCell>
                      <span className="is-positive">+{entriesFor(row.id)}</span>{" "}
                      /{" "}
                      <span className="is-negative">-{outputsFor(row.id)}</span>
                    </TableCell>
                    {canViewCosts && (
                      <>
                        <TableCell>
                          <strong>{formatCurrency(row.costMxn)}</strong>
                          <small>USD ${row.costUsd.toFixed(2)}</small>
                        </TableCell>
                        <TableCell>{formatCurrency(row.partnerCost)}</TableCell>
                      </>
                    )}
                    <TableCell>{formatCurrency(row.retailPrice)}</TableCell>
                    <TableCell>
                      {row.source === "SUPPLY" ? (
                        <button
                          type="button"
                          ...
                          className={`warehouse-visibility-switch ${row.visible ? "is-on" : ""}`}
                          disabled={!canManage}
                          onClick={() => onToggleVisibility(row.id)}
                        >
                          <span>{row.visible ? "VISIBLE" : "OCULTO"}</span>
                          <i />
                        </button>
                      ) : (
                      ...
                      {row.source === "SUPPLY" && canManage ? (
                        <div className="warehouse-row-actions">
                          <Button
                            ...
                            onClick={() =>
                              setItemDraft({
                                ...supplies.find((item) => item.id === row.id)!,
                              })
                            }
                          >
                            <Pencil size={14} />
          ...
          <DialogHeader>
            <DialogTitle>
              {supplies.some((item) => item.id === itemDraft?.id)
                ? "Editar producto de bodega"
                : "Agregar producto a bodega"}
            </DialogTitle>
            <DialogDescription>
              Captura proveedor, costos, precio socio, unidad, presentación y
              límites de stock.
            </DialogDescription>
          </DialogHeader>
          {itemDraft && (
            <div className="warehouse-item-form">
              ...
              <div className="field-stack warehouse-item-image">
      ...
      <Dialog open={restockOpen} onOpenChange={setRestockOpen}>
        <DialogContent className="warehouse-restock-dialog sm:max-w-[820px]">
          <DialogHeader>
            <DialogTitle>Generar pedido de resurtido</DialogTitle>
            <DialogDescription>
              La propuesta completa cada artículo hasta su stock máximo. Puedes
              editar cantidades antes de solicitar aprobación.
            </DialogDescription>
          </DialogHeader>
          <div className="field-stack">
            <Label>Proveedor</Label>
            <Select
              ...
              onValueChange={selectRestockSupplier}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {suppliers
                  ...
                  .map((supplier) => (
                    <SelectItem key={supplier.id} value={supplier.id}>
                      {supplier.folio} · {supplier.businessName}
                    </SelectItem>
                  ))}
              </SelectContent>
            </Select>
          </div>
          <div className="warehouse-restock-lines">
              ...
              return (
                <div key={line.id}>
                  <span>
                    <strong>{row?.name}</strong>
                    <small>
                      {row?.sku} · stock {row?.quantity} / máximo{" "}
                      {row?.stockMax} · {row?.presentation}
                    </small>
                  </span>
                  <Input
                    type="number"
            ...
            {restockLines.length === 0 && (
              <p>
                <AlertTriangle size={17} /> El proveedor no tiene productos por
                debajo del stock máximo.
              </p>
            )}
          </div>
          <div className="field-stack">
            ...
            <Textarea
              ...
              onChange={(event) => setRestockComment(event.target.value)}
            />
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!restockSupplierId || restockLines.length === 0}
            >
              <PackagePlus size={16} /> Solicitar doble aprobación
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

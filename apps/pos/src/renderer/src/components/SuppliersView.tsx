...
interface SuppliersViewProps {
  ...
  onSaveSupplier: (supplier: WarehouseSupplier) => boolean | Promise<boolean>;
  onToggleSupplier: (id: string) => void;
  onDeleteSupplier: (id: string) => void;
  onSaveItem: (item: WarehouseSupplyItem) => boolean | Promise<boolean>;
...
export function SuppliersView({
  ...
  const supplierItems = (supplierId: string) => [
    ...products
      .filter(
        (product) =>
          product.kind === "PRODUCT" && product.supplierId === supplierId,
      )
      .map((product) => ({
        id: product.id,
        name: product.name,
  ...
  const saveSupplier = async () => {
    if (!supplierDraft || !(await onSaveSupplier(supplierDraft))) return;
  ...
  const saveItem = async () => {
    ...
    if (!(await onSaveItem(normalized))) return;
    setItemDraft(null);
  };
  ...
  const importTemplate = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const [index, row] of rows.entries()) {
        ...
        if (await onSaveSupplier(supplier)) created += 1;
      }
  ...
  return (
    <div className="suppliers-view view-stack">
                        ...
                        {canManage && (
                          <>
                            ...
                            <Button
                              ...
                              aria-label={`${supplier.active ? "Inactivar" : "Activar"} ${supplier.businessName}`}
                              title={supplier.active ? "Inactivar" : "Activar"}
                            >
                              {supplier.active ? (
                                <PowerOff size={15} />
                              ) : (
                            ...
                            <Button
                              ...
                              aria-label={`Borrar ${supplier.businessName}`}
                              title="Borrar"
                            >
                              <Trash2 size={15} />
                            </Button>
      ...
      >
        <DialogContent className="sm:max-w-[760px]">
          <DialogHeader>
            ...
            <DialogDescription>
              El folio y los datos fiscales alimentarán pedidos, costos e
              históricos autorizados.
            </DialogDescription>
          </DialogHeader>
          {supplierDraft && (
      ...
      >
        <DialogContent className="supplier-detail-dialog sm:max-w-[900px]">
          <DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-[800px]">
          <DialogHeader>
            ...
            <DialogDescription>
              El artículo se agregará a existencias de bodega y estará
              disponible en listas y resurtidos.
            </DialogDescription>
          </DialogHeader>
          {itemDraft && (

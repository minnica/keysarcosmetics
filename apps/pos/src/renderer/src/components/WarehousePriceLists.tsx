...
interface WarehousePriceListsProps {
  ...
  clients: Client[];
  canManage: boolean;
  onSave: (list: WarehousePriceList) => boolean | Promise<boolean>;
...
export function WarehousePriceLists({
  ...
  const save = async () => {
    if (!draft || !(await onSave(draft))) return;
    setDialogOpen(false);
    setDraft(null);
  };
      ...
      <section className="warehouse-price-list-metrics">
        ...
        <Card>
          <CardContent className="warehouse-price-list-metric-content">
            ...
            <small>Con lista personalizada</small>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="warehouse-price-list-metric-content">
      ...
      <Card className="warehouse-panel">
        <CardContent>
          <div className="warehouse-panel-heading">
            <div>
              <span>PRECIOS SOCIO POR SUCURSAL</span>
              <h2>Listas de precios</h2>
              <p>
                Asigna precios MXN/USD por cliente y sucursal. Los pedidos
                guardan un snapshot histórico.
              </p>
            </div>
            {canManage && (
                  ...
                  <span>
                    ...
                    <small>
                      {list.clientIds.length > 0
                        ? `${list.clientIds.length} clientes asignados`
                ...
                <div>
                  ...
                  <span>
                    {list.clientIds.length > 0
                      ? clients
                          .filter((client) =>
                            list.clientIds.includes(client.id),
                          )
                          .map(
                            (client) =>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="warehouse-price-list-dialog sm:max-w-[1000px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Configura alcance y precios. Un cliente específico podrá elegir
              esta lista únicamente en las sucursales asignadas.
            </DialogDescription>
          </DialogHeader>
          {draft && (
            <div className="warehouse-price-list-form">
              ...
              <section>
                <Label>
                  Clientes asignados{" "}
                  <small>Sin selección = lista general</small>
                </Label>
                <div className="warehouse-price-selector is-clients">
                  {clients.map((client) => (
                    <button
                      ...
                      key={client.id}
                      className={
                        draft.clientIds.includes(client.id) ? "is-selected" : ""
                      }
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Precio socio MXN</TableHead>
                      <TableHead>Precio socio USD</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>

...
interface PosLoginScreenProps {
  ...
  allowMasterBranchSelection?: boolean;
  onLogin: (credentials: {
    company: string;
    username: string;
    password: string;
...
export function PosLoginScreen({
  ...
  allowMasterBranchSelection = !terminalManaged,
  onLogin,
}: PosLoginScreenProps) {
  ...
  const submit = async () => {
    ...
    try {
      const result = await onLogin({
        ...
        password,
        requestedBranch:
          looksMaster && allowMasterBranchSelection ? branch : fixedBranch,
      });
      setError(result ?? "");
    } finally {
      setSubmitting(false);
  ...
  return (
    <main className="software-login-screen">
      <section className="software-login-visual">
        <div className="software-login-orbit" aria-hidden="true">
          <i />
          <i />
          <i />
        </div>
        <span className="software-suite-mark">
          <Boxes size={22} /> RETAIL OPERATIONS SUITE
        </span>
        <div className="software-clock">
          <strong>{clockFormatter.format(now)}</strong>
          <span>
            {(english ? dateFormatterEnglish : dateFormatter).format(now)}
          </span>
        </div>
        <div className="software-login-summary">
          <span>
            <ShieldCheck size={17} />{" "}
            {english
              ? "Session protected by user and location"
              : "Sesión protegida por usuario y sucursal"}
          </span>
          <span>
            <ScanLine size={17} />{" "}
            {english
              ? "Inventory audit at opening and closing"
              : "Auditoría de inventario al abrir y cerrar"}
          </span>
          <span>
            <Clock3 size={17} />{" "}
            {english
              ? "First access records attendance"
              : "El primer acceso registra asistencia"}
          </span>
        </div>
      </section>
      ...
      <section className="software-login-form-panel">
        <div className="software-login-form">
          <span className="section-kicker">
            {english ? "SYSTEM ACCESS" : "ACCESO AL SISTEMA"}
          </span>
          <h1>{english ? "Start your day" : "Iniciar jornada"}</h1>
          <p>
            {english
              ? "Identify the company, your access alias and today's operating terminal."
              : "Identifica la empresa, tu alias de acceso y la terminal donde operarás hoy."}
          </p>

          <div className="field-stack">
            <Label>{english ? "Company name" : "Nombre de la empresa"}</Label>
            <div className="software-login-input">
              <Building2 size={17} />
              <Input
                value={company}
                onChange={(event) => setCompany(event.target.value)}
                placeholder={english ? "Company" : "Empresa"}
              />
            </div>
          </div>
          <div className="field-stack">
            ...
            <div className="software-login-input">
              ...
              <Input
                ...
                placeholder={english ? "Employee alias" : "Alias del vendedor"}
          ...
          <div className="field-stack">
            <Label>
              {english ? "Password or code" : "Contraseña o código"}
            </Label>
            <div className="software-login-input">
              <KeyRound size={17} />
              <Input
                ...
                placeholder={english ? "Access code" : "Código de acceso"}
          ...
          {looksMaster && allowMasterBranchSelection ? (
            <div className="field-stack">
              <Label>
                {english
                  ? "Work location · master access"
                  : "Sucursal de trabajo · acceso master"}
              </Label>
            ...
            <div className="software-fixed-branch">
              ...
              <span>
                <small>
                  {english
                    ? "FIXED LOCATION FOR THIS COMPUTER"
                    : "SUCURSAL FIJA DE ESTA COMPUTADORA"}
                </small>
                <strong>{fixedBranch}</strong>
              </span>
              <LockKeyhole size={15} />
            </div>
          )}
          ...
          {error && (
            <div className="software-login-error">
              <AlertTriangle size={16} /> {error}
            </div>
          )}
          <Button
            type="button"
            className="software-login-submit"
            onClick={() => void submit()}
            disabled={
              submitting ||
              !company.trim() ||
...
export function InventoryCountScreen({
  ...
  const [confirmationMode, setConfirmationMode] = useState<"COUNT" | "SKIP">(
    "COUNT",
  );
  const [masterCode, setMasterCode] = useState("");
  const counted = products.filter(
    (product) => counts[product.id] !== undefined && counts[product.id] !== "",
  ).length;
  ...
  const differences = products.filter((product) => {
    ...
    return (
      value !== undefined &&
      value !== "" &&
      Number(value) !== (expectedStock[product.id] ?? 0)
    );
  }).length;
  const correctCounts = counted - differences;
  const countedPercentage =
    products.length > 0 ? Math.round((counted / products.length) * 100) : 0;
  const correctPercentage =
    products.length > 0
      ? Math.round((correctCounts / products.length) * 100)
      : 0;
  const errorPercentage =
    products.length > 0 ? Math.round((differences / products.length) * 100) : 0;
    ...
    <main className="inventory-count-screen">
      ...
      <header className="inventory-count-header">
          ...
          <h1>
            {mode === "OPENING"
              ...
              : english
                ...
                : "Conteo final de inventario"}
          </h1>
          <p>
            {branch} · {user.name} ·{" "}
            {english
              ? "Enter the physical quantity of each product."
              : "Captura la existencia física de cada producto."}
          </p>
        </div>
        <div className="inventory-count-progress">
          ...
          <i>
            <b style={{ width: `${countedPercentage}%` }} />
          </i>
      ...
      <div className="inventory-count-legend">
        {showDifferences ? (
          <>
            <span>
              <i className="is-match" />{" "}
              {english ? "Matches system" : "Coincide con sistema"}
            </span>
            <span>
              <i className="is-error" />{" "}
              {english ? "Inventory difference" : "Diferencia de inventario"}
            </span>
            {differences > 0 && (
          ...
          <span>
            <ShieldCheck size={15} />{" "}
            {english
              ? "Blind count · differences are protected for administration"
              : "Conteo ciego · las diferencias están protegidas para administración"}
          </span>
        )}
      </div>
      ...
      <section className="inventory-count-grid">
        {products.map((product) => {
          ...
          return (
            <article
              className={`inventory-count-product ${hasValue ? (matches ? "is-match" : "is-error") : ""}`}
              key={product.id}
            >
                ...
                <strong>
                  {showDifferences ? (
                    ...
                    <LockKeyhole
                      size={20}
                      aria-label={
                        english ? "Protected stock" : "Existencia protegida"
                      }
                    />
                  )}
                </strong>
                  ...
                  <span>
                    {matches
                      ...
                      : showDifferences
                        ...
                        : english
                          ? "Review your inventory or count the product again."
                          : "Revisa tu inventario o vuelve a contar tu producto."}
                  </span>
                </div>
              )}
            </article>
      ...
      <section className="inventory-count-comments">
        <div>
          <span className="section-kicker">
            {english ? "COUNT LOG" : "BITÁCORA DEL CONTEO"}
          </span>
          <h2>{english ? "Comments" : "Comentarios"}</h2>
          <p>
            {english
              ? `Record differences, damaged products, incidents or any relevant ${mode === "OPENING" ? "opening" : "closing"} observation.`
              : `Registra diferencias, productos dañados, incidencias o cualquier observación relevante ${mode === "OPENING" ? "de la apertura" : "del cierre"}.`}
          </p>
        </div>
        <Textarea
          value={comment}
          ...
          placeholder={
            mode === "OPENING"
              ? english
                ? "E.g. The location received one damaged item…"
                : "Ej. Se recibió la sucursal con una pieza dañada…"
              : english
                ? "E.g. The difference was reviewed with administration…"
                : "Ej. La diferencia fue revisada con administración…"
          }
          maxLength={500}
          aria-label={
            english
              ? `${mode === "OPENING" ? "Opening" : "Closing"} count comments`
              : `Comentarios del conteo de ${mode === "OPENING" ? "apertura" : "cierre"}`
          }
        />
      ...
      <footer className="inventory-count-footer">
        <span>
          ...
          <b>{counted}</b>{" "}
          {english ? "products entered" : "productos capturados"}
          {showDifferences && (
            <>
              {" "}
              · <b>{differences}</b>{" "}
              {english ? "with differences" : "con diferencia"}
            </>
          )}
        </span>
        {canSkip && (
          <Button
            ...
            variant="outline"
            onClick={() => {
              if (import.meta.env.VITE_POS_VISUAL_FIXTURE === "1") {
                onComplete(buildLines(), true, comment.trim());
                return;
              }
              setConfirmationMode("SKIP");
              setMasterCode("");
        ...
        <Button
          type="button"
          className="open-day-button"
          disabled={!allCounted}
          onClick={() => {
            setConfirmationMode("COUNT");
            setConfirmationOpen(true);
          }}
        >
          {mode === "OPENING"
            ? "Open Day"
            : english
              ? "Save and continue to closing"
              : "Guardar y continuar al corte"}{" "}
          <ArrowRight size={17} />
        </Button>
      </footer>
          ...
          <DialogHeader>
            <DialogTitle>
              {confirmationMode === "SKIP"
                ? "Autorizar omisión del conteo"
                : english
                  ? "Are you sure you want to submit this data?"
                  : "¿Estás seguro de enviar estos datos?"}
            </DialogTitle>
            <DialogDescription>
              {confirmationMode === "SKIP" ? (
                "Confirma la acción con un código master. La autorización quedará ligada a esta jornada y terminal."
              ) : english ? (
                `This will be the first saved ${mode === "OPENING" ? "opening" : "closing"} count. Review all quantities before continuing.`
              ) : (
                <>
                  Este será el primer guardado del conteo de{" "}
                  {mode === "OPENING" ? "apertura" : "cierre"}. Revisa las
            ...
            <span>
              ...
              <small>
                {showDifferences
                  ...
                  : english
                    ? "Numeric differences will remain protected for administration."
                    : "Las diferencias numéricas permanecerán protegidas para administración."}
              </small>
            </span>
          </div>
          {confirmationMode === "SKIP" && (
            <div className="field-stack">
              <Label htmlFor="inventory-skip-master-code">Código master</Label>
              <Input
                id="inventory-skip-master-code"
                type="password"
                inputMode="numeric"
                maxLength={4}
                value={masterCode}
                onChange={(event) =>
                  setMasterCode(event.target.value.replace(/\D/g, ""))
                }
            ...
            <Button
              ...
              onClick={() => {
                ...
                onComplete(
                  ...
                  confirmationMode === "SKIP",
                  comment.trim(),
                  confirmationMode === "SKIP" ? masterCode : undefined,
                );
              }}
              disabled={confirmationMode === "SKIP" && masterCode.length !== 4}
            >
              <ShieldCheck size={16} />{" "}
              {english ? "Confirm and submit" : "Confirmar y enviar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function MasterDashboard({
  ...
  const [dashboardBranch, setDashboardBranch] = useState(
    canViewAllBranches ? "ALL" : session.branch,
  );
  const [inventoryReportBranch, setInventoryReportBranch] = useState(
    session.branch,
  );
  useEffect(() => {
    ...
    if (
      dashboardBranch !== "ALL" &&
      !availableBranches.includes(dashboardBranch)
    ) {
  ...
  const scopeLabel =
    scopedBranch === "ALL" ? "Todas las sucursales" : scopedBranch;
  ...
  const scopedMovements = movements.filter(
    (movement) =>
      ...
      (scopedBranch === "ALL" ||
        movement.sourceBranch === scopedBranch ||
        movement.destinationBranch === scopedBranch),
  );
  ...
  const scopedAppointments = appointments.filter(
    (appointment) =>
      ...
      (businessDate(appointment.recordedAtIso) === sessionBusinessDate ||
        appointment.date === sessionBusinessDate),
  );
  const sales = scopedTickets.reduce((sum, ticket) => sum + ticket.total, 0);
  const collected = scopedTickets.reduce(
    (sum, ticket) => sum + ticket.amountPaid,
    0,
  );
  const expenseTotal = scopedExpenses.reduce(
    (sum, expense) => sum + expense.amount,
    0,
  );
  const discountTotal = scopedTickets.reduce(
    (sum, ticket) => sum + ticket.discountAmount,
    0,
  );
  const unitsSold = scopedTickets.reduce(
    (sum, ticket) =>
      sum +
      ticket.products.reduce((lineSum, line) => lineSum + line.quantity, 0),
    0,
  );
  const writeOffs = scopedMovements
    .filter(
      (movement) =>
  ...
  const sellerRanking = Array.from(
    scopedTickets
      ...
      .reduce<
        Map<string, number>
      >((map, sale) => map.set(sale.sellerName, (map.get(sale.sellerName) ?? 0) + sale.amount), new Map()),
    ([name, total]) => ({ name, total }),
  ).sort((left, right) => right.total - left.total);
  const serviceSales = Array.from(
    scopedTickets
      .reduce<Map<string, { name: string; quantity: number; total: number }>>(
        (summary, ticket) => {
          ticket.products.forEach((line) => {
            ...
            if (product?.kind !== "SERVICE" || line.name.endsWith(" · REGALO"))
              return;
  ...
  const paymentTotals = Array.from(
    scopedTickets.reduce<Map<string, number>>((summary, ticket) => {
      const entries =
      ...
      entries.forEach((payment) =>
        summary.set(
          payment.methodId,
          (summary.get(payment.methodId) ?? 0) + payment.amount,
        ),
      );
      return summary;
    }, new Map()),
  ...
  const [sessionYear = "", sessionMonth = "01"] =
    sessionBusinessDate.split("-");
  ...
  const getLatestBranchAudit = (branch: string, type: "OPENING" | "CLOSING") =>
    inventoryAudits
      .filter(
        (audit) =>
          audit.branch === branch &&
  ...
  const buildInventoryRowsForBranch = (branch: string) => {
    ...
    return branchOpeningAudit.lines.map((line) => {
      ...
      const movementNet = productMovements.reduce((net, movement) => {
        ...
        if (movement.direction === "REMOVE" && movement.sourceBranch === branch)
          return net - movement.quantity;
      ...
      const operation = productMovements.reduce(
        (summary, movement) => {
          const reason =
            `${movement.reason} ${movement.comment}`.toLocaleLowerCase("es-MX");
  ...
  const openingAuditBranchCount = availableBranches.filter(
    (branch) =>
      Boolean(getLatestBranchAudit(branch, "OPENING")) ||
      branch === session.branch,
  ).length;
  const closingAuditBranchCount = availableBranches.filter((branch) =>
    Boolean(getLatestBranchAudit(branch, "CLOSING")),
  ).length;
  const maxSeller = Math.max(1, ...sellerRanking.map((seller) => seller.total));
  const movementTotals = scopedMovements.reduce(
    (summary, movement) => {
      const reason = `${movement.reason} ${movement.comment}`.toLocaleLowerCase(
        "es-MX",
      );
  ...
  const buildErrorExportRows = (rows: typeof inventoryRows) =>
    rows.map((row) => ({
  ...
  const exportErrorsExcel = async (
    ...
    XLSX.writeFile(
      workbook,
      `reconteo-${scope.toLocaleLowerCase("es-MX").replace(/\s+/g, "-")}.xlsx`,
    );
  };
  ...
  const exportErrorsPdf = async (
    ...
    doc.setFontSize(9);
    doc.text(
      `Alcance: ${scope} · Productos con diferencia: ${rows.length} · Apertura y cierre para validación física`,
      36,
      57,
    );
    const headers = [
      ...
      "Damage",
      "Gift",
      ...(showCosts ? ["Costo MXN", "Impacto MXN"] : []),
    ];
    autoTable(doc, {
      startY: 73,
      head: [headers],
      body: rows.map((row) => [
        ...
        ...(showCosts
          ? [
              formatCurrency(row.unitCostMxn),
              formatCurrency(row.auditDifference * row.unitCostMxn),
            ]
          : []),
      ]),
    ...
    doc.save(
      `reconteo-${scope.toLocaleLowerCase("es-MX").replace(/\s+/g, "-")}.pdf`,
    );
  };
  ...
  return (
    <div className="master-dashboard-view">
      <section className="master-dashboard-hero">
        <div>
          <span className="section-kicker">
            CONTROL EJECUTIVO · TIEMPO REAL
          </span>
          <h2>Dashboard de jornada</h2>
          <p>
            {scopeLabel} · Abierto por {openingAudit.createdByName}
          </p>
          {showInventoryDifferences && openingAudit.comment && (
            <small>Nota de apertura: {openingAudit.comment}</small>
          )}
          {showInventoryDifferences && closingAudit?.comment && (
            <small>Nota de cierre: {closingAudit.comment}</small>
          )}
        </div>
        <div className="dashboard-scope-control">
          {canViewAllBranches ? (
            <>
              <span>ALCANCE MASTER</span>
              <Select
                value={dashboardBranch}
                onValueChange={setDashboardBranch}
              >
                <SelectTrigger aria-label="Alcance del dashboard">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <Badge variant="outline">
              <Store size={13} /> Sucursal fija · {session.branch}
            </Badge>
          )}
          <Badge variant="outline">
            {session.status === "OPEN" ? "OPEN DAY" : "CLOSED"}
          </Badge>
        </div>
      </section>
      <section className="master-dashboard-metrics">
        <article>
          ...
          <small>{scopedTickets.length} tickets</small>
        </article>
        <article>
          <WalletCards size={18} />
          <span>Cobrado</span>
          <strong>{formatCurrency(collected)}</strong>
          <small>{formatCurrency(sales - collected)} pendiente</small>
        </article>
        <article>
          <TrendingUp size={18} />
          <span>Flujo después de gastos</span>
          <strong>{formatCurrency(collected - expenseTotal)}</strong>
          <small>-{formatCurrency(expenseTotal)} gastos</small>
        </article>
        <article>
          <PackageCheck size={18} />
          <span>Productos vendidos</span>
          <strong>{unitsSold}</strong>
          <small>{writeOffs} bajas adicionales</small>
        </article>
        <article>
          <BadgePercent size={18} />
          <span>Descuentos aplicados</span>
          <strong>{formatCurrency(discountTotal)}</strong>
          <small>Promociones y ajustes del día</small>
        </article>
      </section>
      <section className="master-dashboard-panels">
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <UsersRound size={18} /> Venta y distribución por vendedor
              </span>
              <Badge variant="outline">{scopeLabel}</Badge>
            </div>
            <div className="master-seller-chart">
                ...
                <div key={seller.name}>
                  <span>
                    ...
                    <strong>
                      ...
                      {sales > 0
                        ? ((seller.total / sales) * 100).toFixed(1)
                        : "0.0"}
                      %
                    </strong>
                  </span>
                  <i>
                    <b
                      style={{ width: `${(seller.total / maxSeller) * 100}%` }}
                    />
                  </i>
                </div>
              ))}
              {sellerRanking.length === 0 && (
                <p>Sin ventas registradas en el alcance seleccionado.</p>
              )}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <Boxes size={18} /> Movimientos de producto
              </span>
              <Badge variant="outline">TIEMPO REAL</Badge>
            </div>
            <div className="master-movement-summary is-detailed">
              <span>
                <PackagePlus size={19} />
                <b>Entradas</b>
              ...
              <span>
                <ShoppingBag size={19} />
                <b>Ventas</b>
                <strong>{movementTotals.sales}</strong>
              </span>
              <span>
                <FlaskConical size={19} />
                <b>Demos</b>
                <strong>{movementTotals.demos}</strong>
              </span>
              <span>
                <PackageMinus size={19} />
                <b>Bajas</b>
                <strong>{movementTotals.writeOffs}</strong>
              </span>
              <span>
                <AlertTriangle size={19} />
                <b>Lost / Damage</b>
                <strong>{movementTotals.lost + movementTotals.damage}</strong>
              </span>
              <span>
                <Gift size={19} />
                <b>Gift</b>
                <strong>{movementTotals.gifts}</strong>
              </span>
              <span>
                <Boxes size={19} />
                <b>Transferencias</b>
                <strong>{movementTotals.transfers}</strong>
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      <section className="dashboard-operational-reports">
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <Sparkles size={18} /> Servicios vendidos y cortesías
              </span>
              <Badge variant="outline">
                {serviceSales.reduce(
            ...
            <div className="dashboard-report-kpis">
              <span>
                <strong>
                  {formatCurrency(
                    serviceSales.reduce(
                ...
                <small>Venta de servicios</small>
              </span>
              <span>
                <strong>{courtesyAppointments.length}</strong>
                <small>Cortesías registradas</small>
              </span>
            </div>
            <div className="dashboard-report-list">
              {serviceSales.map((service) => (
                <div key={service.name}>
                  <span>
                    <b>{service.name}</b>
                    <small>{service.quantity} vendidos</small>
                  </span>
                  <strong>{formatCurrency(service.total)}</strong>
                </div>
              ))}
              {serviceSales.length === 0 && (
                <p>Sin servicios vendidos en el alcance seleccionado.</p>
              )}
              {courtesyAppointments.map((appointment) => (
                <div className="is-courtesy" key={appointment.id}>
                  <span>
                    <b>{appointment.service}</b>
                    <small>
                      {appointment.clientName} · {appointment.branch}
                    </small>
                  </span>
        ...
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <CalendarDays size={18} /> Citas generadas
              </span>
              <Badge variant="outline">
                {scopedAppointments.length} REGISTROS
              </Badge>
            </div>
            <div className="dashboard-appointment-kpis">
              <span>
                <HeartHandshake size={17} />
                <b>{courtesyAppointments.length}</b>
                <small>Cortesías</small>
              </span>
              <span>
                <CalendarDays size={17} />
                <b>{nextSessionAppointments.length}</b>
                <small>Próximas</small>
              </span>
              <span>
                <AlertTriangle size={17} />
                <b>{missingAppointments.length}</b>
                <small>Sin cita</small>
              </span>
            </div>
            <div className="dashboard-report-list">
              {appointmentServices.map((service) => (
                <div key={service.name}>
                  <span>
                    <b>{service.name}</b>
                    <small>Agenda y cortesías</small>
                  </span>
                  <strong>{service.quantity}</strong>
                </div>
              ))}
              {appointmentServices.length === 0 && (
                <p>Sin citas o cortesías generadas para este alcance.</p>
              )}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <CreditCard size={18} /> Total por método de pago
              </span>
              <Badge variant="outline">
                {formatCurrency(
            ...
            <div className="dashboard-payment-list">
              {paymentTotals.map((payment) => (
                <div key={payment.methodId}>
                  ...
                  <i>
                    <b
                      style={{
                        width: `${collected > 0 ? Math.min(100, (payment.total / collected) * 100) : 0}%`,
                      }}
                    />
                  </i>
                  <small>
                    {collected > 0
                      ? ((payment.total / collected) * 100).toFixed(1)
                      : "0.0"}
                    % del cobro
                  </small>
                </div>
              ...
              {paymentTotals.length === 0 && (
                <p>Sin cobros registrados en el alcance seleccionado.</p>
              )}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <WalletCards size={18} /> Crédito y meses sin intereses
              </span>
              <Badge variant="outline">{formatCurrency(creditCollected)}</Badge>
            </div>
            <div className="dashboard-payment-list">
              {installmentTotals.map((item) => (
                <div key={item.months}>
                  <span>
                    <CreditCard size={15} />
                    <b>
                  ...
                  <i>
                    <b
                      style={{
                        width: `${Math.max(3, (item.count / Math.max(1, installmentTotals[0]?.count ?? 1)) * 100)}%`,
                      }}
                    />
                  </i>
                  <small>
                    {formatCurrency(item.total)} · {scopeLabel} ·{" "}
                    {sessionBusinessDate}
                  </small>
                </div>
              ))}
              {installmentTotals.length === 0 && (
                <p>Sin compras con tarjeta de crédito en el alcance del día.</p>
              )}
            </div>
          </CardContent>
        </Card>
      ...
      {showMembershipReport && (
        <Card className="dashboard-membership-card">
          <CardContent>
            <div className="master-dashboard-panel-heading">
              <span>
                <Crown size={19} /> Reporte ejecutivo de membresías
              </span>
            ...
            <div className="dashboard-membership-layout">
              <div className="dashboard-membership-kpis">
                <span>
                  <strong>{formatCurrency(membershipRevenue)}</strong>
                  <small>Venta acumulada</small>
                </span>
                <span>
                  <strong>{activeMembershipCards.length}</strong>
                  <small>Membresías activas</small>
                </span>
                <span>
                  <strong>{membershipSessionsAvailable}</strong>
                  <small>Sesiones disponibles</small>
                </span>
                <span className={membershipRenewalAlerts > 0 ? "is-alert" : ""}>
                  <strong>{membershipRenewalAlerts}</strong>
                  <small>Por terminar</small>
                </span>
              </div>
              <div className="dashboard-membership-leaders">
                <div>
                  <Store size={17} />
                  <span>
                    <small>SUCURSAL LÍDER</small>
                    <strong>
                      {membershipBranchRanking[0]?.name ?? "Sin ventas"}
                    </strong>
                  </span>
                  <b>
                    {membershipBranchRanking[0]
                      ? `${membershipBranchRanking[0].sales} ${membershipBranchRanking[0].sales === 1 ? "venta" : "ventas"}`
                      : "—"}
                  </b>
                </div>
                <div>
                  <Trophy size={17} />
                  <span>
                    <small>VENDEDOR LÍDER</small>
                    <strong>
                      {membershipSellerRanking[0]?.name ?? "Sin ventas"}
                    </strong>
                  </span>
                  <b>
                    {membershipSellerRanking[0]
                      ? `${membershipSellerRanking[0].sales} ${membershipSellerRanking[0].sales === 1 ? "membresía" : "membresías"}`
                      : "—"}
                  </b>
                </div>
              </div>
              <div className="dashboard-membership-podium">
                <header>
                  <span>
                    <Trophy size={16} /> TOP 3 DEL ÚLTIMO MES CERRADO
                  </span>
                  <Badge
                    variant="outline"
                    className="dashboard-membership-period-badge"
                  >
                ...
                {closedMonthSellerRanking.map((seller, index) => (
                  <div key={seller.name}>
                    ...
                    <span>
                      ...
                      <small>
                        {seller.sales} membresía{seller.sales === 1 ? "" : "s"}
                      </small>
                    </span>
                    <b>{formatCurrency(seller.revenue)}</b>
                  </div>
                ))}
                {closedMonthSellerRanking.length === 0 && (
                  <p>Sin ventas de membresías en el último mes cerrado.</p>
                )}
              </div>
            </div>
          </CardContent>
      ...
      <Card className="master-inventory-audit-card">
        <CardContent>
          <div className="master-dashboard-panel-heading master-audit-heading">
            <span>
              <ScanLine size={19} /> Conteo y trazabilidad de inventario
            </span>
            <div className="master-audit-actions">
              {canViewAllBranches && scopedBranch === "ALL" && (
                <Select
                  value={inventoryReportBranch}
                  onValueChange={setInventoryReportBranch}
                >
                  <SelectTrigger
                    className="master-audit-branch-select"
                    aria-label="Sucursal del reporte de conteo"
                  >
                    <Store size={14} />
                    <SelectValue />
                  </SelectTrigger>
              ...
              <Badge variant="outline">
                <Store size={12} /> {selectedInventoryBranch}
              </Badge>
              {showInventoryDifferences ? (
                <Badge
                  variant="outline"
                  className={errorRows.length > 0 ? "is-error" : ""}
                >
                  {errorRows.length} CON ERROR
                </Badge>
              ) : (
                <Badge variant="outline">
                  <LockKeyhole size={12} /> INVENTARIO REAL PROTEGIDO
                </Badge>
              )}
              {showInventoryDifferences && (
                <Button
                  ...
                  onClick={() => void exportErrorsExcel()}
                >
                  <FileSpreadsheet size={15} /> Excel errores
                </Button>
              )}
              {showInventoryDifferences && (
                <Button
                  ...
                  variant="outline"
                  disabled={errorRows.length === 0}
                  onClick={() => void exportErrorsPdf()}
                >
                  <FileDown size={15} /> PDF errores
                </Button>
              )}
              {showInventoryDifferences && scopedBranch === "ALL" && (
                <Button
                  type="button"
                  size="sm"
                  variant="outline"
                  disabled={generalErrorRows.length === 0}
                  onClick={() =>
                    void exportErrorsExcel(
                      generalErrorRows,
                      "todas-las-sucursales",
                    )
                  }
                >
                  <FileSpreadsheet size={15} /> Excel general
                </Button>
              )}
              {showInventoryDifferences && scopedBranch === "ALL" && (
                <Button
                  ...
                  disabled={generalErrorRows.length === 0}
                  onClick={() =>
                    void exportErrorsPdf(
                      generalErrorRows,
                      "todas-las-sucursales",
                    )
                  }
                >
                  <FileDown size={15} /> PDF general
                </Button>
              )}
            </div>
          </div>
          <p className="master-audit-copy">
            {showInventoryDifferences
              ? `Vista autorizada de ${selectedInventoryBranch}: compara su conteo físico, existencia real y diferencias. Las descargas por tienda y el consolidado general incluyen sólo productos con error.`
              : `Vista operativa de ${selectedInventoryBranch}: muestra exclusivamente conteos registrados y movimientos generados en tiempo real. La existencia física, diferencias y costos permanecen protegidos.`}
          </p>
          <div className="master-audit-status">
            <Badge
              variant="outline"
              className={selectedOpeningAudit ? "is-ready" : "is-pending"}
            >
              {selectedOpeningAudit ? "APERTURA REGISTRADA" : "SIN APERTURA"}
            </Badge>
            <Badge
              variant="outline"
              className={selectedClosingAudit ? "is-ready" : "is-pending"}
            >
              {selectedClosingAudit
                ? "CONTEO FINAL REGISTRADO"
          ...
          {scopedBranch === "ALL" && (
            <div className="master-audit-summary">
              <span>
                <small>APERTURAS REGISTRADAS</small>
                <strong>
                  {openingAuditBranchCount}/{availableBranches.length}
                </strong>
              </span>
              <span>
                <small>CONTEOS FINALES</small>
                <strong>
                  {closingAuditBranchCount}/{availableBranches.length}
                </strong>
              </span>
              <span className={generalErrorRows.length > 0 ? "is-error" : ""}>
                <small>ERRORES GENERALES</small>
                <strong>
                  {showInventoryDifferences
                    ? generalErrorRows.length
                    : "Protegido"}
                </strong>
              </span>
            </div>
          )}
          {showInventoryDifferences && errorRows.length > 0 && (
            <div className="master-recount-share">
              <Mail size={17} />
              <span>
                <strong>
                  Archivo listo para compartir con {selectedInventoryBranch}
                </strong>
                <small>
                  Descarga únicamente los productos con error para solicitar por
                  correo un nuevo conteo y validar la existencia física.
                </small>
              </span>
            </div>
          )}
          {!selectedOpeningAudit ? (
            <div className="master-audit-empty">
              <ScanLine size={22} />
              <span>
                <strong>
                  Sin conteo de apertura para {selectedInventoryBranch}
                </strong>
                <small>
                  Esta tienda no tiene un conteo registrado en la jornada
                  actual. No se muestran existencias de otra sucursal como
                  sustituto.
                </small>
              </span>
            </div>
            ...
            <div
              className={`master-audit-table ${showInventoryDifferences ? "is-authorized" : "is-operational"}`}
            >
              {showInventoryDifferences ? (
                <table>
                  <thead>
                    <tr>
                      <th>Producto</th>
                      <th>Sistema apertura</th>
                      <th>Conteo apertura</th>
                      <th>Movimientos</th>
                      <th>Esperado actual</th>
                      <th>Existencia real</th>
                      <th>Cierre contado</th>
                      <th>Diferencia</th>
                      {showCosts && (
                        <>
                          <th>Costo MXN</th>
                          <th>Impacto diferencia</th>
                        </>
                      )}
                    </tr>
                  </thead>
                  <tbody>
                    {inventoryRows.map((row) => (
                      <tr
                        className={row.hasError ? "is-error" : "is-match"}
                        key={row.productId}
                      >
                        <td>
                          <img
                        ...
                        <td>{row.expectedCurrent}</td>
                        <td>
                          <strong>{row.realCurrent}</strong>
                        </td>
                        <td>{row.closingLine?.actualStock ?? "—"}</td>
                        <td>
                          <b>
                            {row.auditDifference > 0 ? "+" : ""}
                            {row.auditDifference}
                          </b>
                        </td>
                        {showCosts && (
                          <>
                            <td>{formatCurrency(row.unitCostMxn)}</td>
                            <td>
                              {formatCurrency(
                                row.auditDifference * row.unitCostMxn,
                              )}
                            </td>
                          </>
                        )}
                ...
                <table>
                  ...
                  <tbody>
                    {inventoryRows.map((row) => (
                      <tr className="is-protected" key={row.productId}>
                        <td>
                          <img
                            src={
                              products.find(
                        ...
                        <td>
                          <Badge variant="outline">
                            {selectedOpeningAudit.skipped
                              ? "Omitido por master"
                              : "Registrado"}
                          </Badge>
                        </td>
                        <td>{row.operation.sales}</td>
                        <td>{row.operation.demos}</td>
                        <td>{row.operation.writeOffs}</td>
                        <td>{row.operation.lost}</td>
                        <td>{row.operation.damage}</td>
                        <td>{row.operation.gifts}</td>
                        <td>+{row.operation.entries}</td>
                        <td>
                          {row.operation.transferIn + row.operation.transferOut}
                        </td>
                        <td>
                          {row.movementNet > 0 ? "+" : ""}
                          {row.movementNet}

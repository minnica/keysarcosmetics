...
export function useAppointments(filters: DateFilters) {
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await api.get<{ success: boolean; data: RegistroCita[] }>('/api/envelope/citas', {
        params: filters,
      })
  ...
  const add = useCallback(async (input: AppointmentInput) => {
    try {
      ...
      throw new Error(apiErrorMessage(saveError, 'No se pudo registrar la cita'))
    }
  }, [refetch])

  const update = useCallback(async (id: string, input: AppointmentInput) => {
    try {
      await api.put(`/api/envelope/citas/${id}`, input)
      await refetch()
    } catch (saveError) {
      throw new Error(apiErrorMessage(saveError, 'No se pudo actualizar la cita'))
    }
  }, [refetch])
  ...
  const remove = useCallback(async (id: string) => {
    try {
      ...
      throw new Error(apiErrorMessage(deleteError, 'No se pudo eliminar la cita'))
    }
  }, [refetch])
...
export function useAppointmentReport(filters: ReportFilters) {
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await api.get<{ success: boolean; data: AppointmentReportRow[] }>('/api/envelope/reportes/citas', {
        params: filters,
      })
      setRows(response.data.data)
    } catch (loadError) {
      setError(apiErrorMessage(loadError, 'No se pudo generar el reporte de citas'))
    } finally {
      setLoading(false)
    }
  }, [filters.facialistaId, filters.fechaFin, filters.fechaInicio, filters.sucursalId])

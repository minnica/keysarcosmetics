...
function toRegistroVenta(v: VentaRaw): RegistroVenta {
  return {
    ...
    items: v.detalles.map((d): VentaItem => ({
      id: d.id,
      cantidad: Number(d.cantidad),
      metodoPagoId: d.metodoPagoId,
      // notas está a nivel de Venta en el backend; se propaga al primer item para compatibilidad
...
export function useVentas(): UseVentasReturn {
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null)
    try {
      const { data } = await api.get<{ success: boolean; data: VentaRaw[] }>('/api/envelope/ventas')
  ...
  const add = useCallback(async (registro: RegistroVenta) => {

...
import {
  ...
  AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, Badge, Button, Card, CardContent,
...
export default function AttentionServicesPage() {
  ...
  function open(kind: 'category' | 'subcategory') {
    setName('')
    setCategoryId(kind === 'subcategory' ? categories.find((category) => category.activa)?.id ?? '' : '')
    setDialog(kind)
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 md:mx-0">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2"><Button variant="outline" onClick={() => open('category')}><Plus className="mr-2 h-4 w-4" />Categoría</Button><Button onClick={() => open('subcategory')} disabled={!categories.some((category) => category.activa)}><Plus className="mr-2 h-4 w-4" />Servicio</Button></div>
      </div>
      {error && <p role="alert" className="text-sm text-red-500">{error}</p>}
      {loading && !loaded ? <TableLoadingSkeleton columns={3} rows={5} label="Cargando servicios..." /> : categories.length === 0 ? (
      ...
      ) : <div className="grid gap-4 md:grid-cols-2">{categories.map((category) => <Card key={category.id}><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{category.nombre}</CardTitle><CardDescription>{category.subcategorias.filter((service) => service.activa).length} servicio(s) activo(s)</CardDescription></div><Button variant="ghost" size="icon" className="text-red-600 hover:bg-red-50 hover:text-red-700" disabled={!category.activa} onClick={() => setPending({ kind: 'category', id: category.id, name: category.nombre })} aria-label={`Desactivar ${category.nombre}`}><Trash2 className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-2">{category.subcategorias.length === 0 ? <p className="text-sm text-[color:var(--text-muted)]">Sin servicios registrados.</p> : category.subcategorias.map((service) => <div key={service.id} className="flex items-center justify-between gap-3 border-t border-[color:var(--border-color)] py-2"><span className={!service.activa ? 'text-[color:var(--text-muted)] line-through' : ''}>{service.nombre}</span><div className="flex items-center gap-2">{!service.activa && <Badge variant="secondary">INACTIVO</Badge>}<Button variant="ghost" size="icon" className="h-8 w-8 text-red-600 hover:bg-red-50 hover:text-red-700" disabled={!service.activa || !category.activa} onClick={() => setPending({ kind: 'subcategory', id: service.id, name: service.nombre })} aria-label={`Desactivar ${service.nombre}`}><Trash2 className="h-4 w-4" /></Button></div></div>)}</CardContent></Card>)}</div>}
      <Dialog open={dialog !== null} onOpenChange={(openState) => !openState && setDialog(null)}><DialogContent><DialogHeader><DialogTitle>{dialog === 'category' ? 'Nueva categoría' : 'Nuevo servicio'}</DialogTitle><DialogDescription>{dialog === 'category' ? 'Ejemplo: FACIAL o CORPORAL.' : 'El servicio quedará disponible al seleccionar su categoría en el registro de citas.'}</DialogDescription></DialogHeader><div className="space-y-4 py-2">{dialog === 'subcategory' && <div className="space-y-1.5"><Label>Categoría</Label><Select value={categoryId} onValueChange={setCategoryId}><SelectTrigger><SelectValue placeholder="Selecciona una categoría" /></SelectTrigger><SelectContent>{categories.filter((category) => category.activa).map((category) => <SelectItem key={category.id} value={category.id}>{category.nombre}</SelectItem>)}</SelectContent></Select></div>}<div className="space-y-1.5"><Label htmlFor="service-name">Nombre</Label><Input id="service-name" value={name} onChange={(event) => setName(event.target.value.toUpperCase())} placeholder={dialog === 'category' ? 'EJ. CORPORAL' : 'EJ. MASAJE REDUCTIVO'} /></div></div><DialogFooter><Button variant="outline" onClick={() => setDialog(null)}>Cancelar</Button><Button onClick={() => void submit()} disabled={saving || !name.trim() || (dialog === 'subcategory' && !categoryId)}>{saving ? 'Guardando...' : 'Guardar'}</Button></DialogFooter></DialogContent></Dialog>

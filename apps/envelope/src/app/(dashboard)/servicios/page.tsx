  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 md:mx-0">
      ...
      {loading && !loaded ? <TableLoadingSkeleton columns={3} rows={5} label="Cargando servicios..." /> : activeCategories.length === 0 ? (
        ...
        return <Card key={category.id}><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{category.nombre}</CardTitle><CardDescription>{activeServices.length} servicio(s) activo(s)</CardDescription></div><Button variant="outline" size="icon" className={actionButtonStyles.danger} onClick={() => setPending({ kind: 'category', id: category.id, name: category.nombre })} aria-label={`Desactivar ${category.nombre}`}><Trash2 className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-2">{activeServices.length === 0 ? <p className="text-sm text-[color:var(--text-muted)]">Sin servicios registrados.</p> : activeServices.map((service) => <div key={service.id} className="flex items-center justify-between gap-3 border-t border-[color:var(--border-color)] py-2"><span>{service.nombre}</span><Button variant="outline" size="icon" className={`${actionButtonStyles.danger} h-8 w-8`} onClick={() => setPending({ kind: 'subcategory', id: service.id, name: service.nombre })} aria-label={`Desactivar ${service.nombre}`}><Trash2 className="h-4 w-4" /></Button></div>)}</CardContent></Card>
      })}</div>}

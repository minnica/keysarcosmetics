...
import { PayrollMockProvider } from './bonus-catalog-context'
                  ...
                  return (
                    <SidebarMenuItem key={item.href}>
                      <SidebarMenuButton
                        ...
                        tooltip={item.label}
                        onClick={() => setOpenMobile(false)}
                        className={isActive ? '!bg-[var(--sidebar-active-bg)] !text-[var(--sidebar-active-text)] hover:!bg-[var(--sidebar-active-bg)] hover:!text-[var(--sidebar-active-text)]' : undefined}
                      >
                        <Link href={item.href}>
                          <Icon />
    ...
    <PayrollMockProvider>
      <SidebarProvider>
        ...
        <SidebarInset className="min-w-0 overflow-x-hidden">
          <header className="flex h-12 shrink-0 items-center gap-3 border-b px-4 md:hidden" style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-card)' }}>
            <SidebarTrigger className="text-[var(--text-muted)] hover:text-[var(--text-primary)]" />
            <img src="/logo.svg" alt="Keysar Cosmetics" className="h-6 object-contain" style={{ maxWidth: '100px' }} />
          </header>

...
export function StatusBadge({ status }: { status: Status }) {
  const isSuccess = status === 'APPROVED' || status === 'PAID' || status === 'CONFIRMED'

  return (
    <Badge
      variant={STATUS_VARIANTS[status]}
      className={isSuccess ? 'bg-[#648672] text-white uppercase dark:bg-[#8bb09b] dark:text-[#1a1a1a]' : 'uppercase'}
    >
      {formatStatus(status)}
    </Badge>
  )

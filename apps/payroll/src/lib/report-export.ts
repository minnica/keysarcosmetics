...
function formatForDisplay<T>(column: ExportColumn<T>, value: ExportCellValue): string {
  ...
  if (column.format === 'number' && typeof value === 'number') return new Intl.NumberFormat('es-MX').format(value)
  return uppercase(String(value))
}

function toExcelCell<T>(column: ExportColumn<T>, value: ExportCellValue): CellObject {
  if (value === null || value === undefined || value === '') return { t: 's', v: '—' }
  if (column.format === 'currency' && typeof value === 'number') return { t: 'n', v: value, z: '$#,##0.00' }
  if (column.format === 'percent' && typeof value === 'number') return { t: 'n', v: value / 100, z: '0.00%' }
  if (column.format === 'number' && typeof value === 'number') return { t: 'n', v: value, z: '#,##0' }
...
export async function exportReportToExcel<T>(config: ReportExportConfig<T>): Promise<void> {
  ...
  const titleRows = [[config.title], ...(config.subtitle ? [[config.subtitle]] : [])]
  const bodyRows = config.rows.map((row) => config.columns.map((column) => toExcelCell(column, column.accessor(row))))
  const footerRows = config.footerRow
    ? [config.columns.map((column) => toExcelCell(column, column.accessor(config.footerRow as T)))]
    : []
  ...
  sheet['!cols'] = config.columns.map((column) => ({ wch: Math.max(column.header.length, column.width ?? 12) }))
  sheet['!autofilter'] = {
    ref: XLSX.utils.encode_range({
      s: { r: headerRowIndex, c: 0 },
      e: { r: headerRowIndex + bodyRows.length + footerRows.length, c: Math.max(config.columns.length - 1, 0) },
    }),
  }

  XLSX.utils.book_append_sheet(workbook, sheet, sanitizeSheetName(config.sheetName))
...
export async function exportReportToPdf<T>(config: ReportExportConfig<T>): Promise<void> {
  ...
  const options: UserOptions = {
    ...
    body: config.rows.map((row) => config.columns.map((column) => formatForDisplay(column, column.accessor(row)))),
    theme: 'striped',
    styles: { font: 'helvetica', fontSize: 7.5, cellPadding: 4, overflow: 'linebreak', valign: 'middle' },
    headStyles: { fillColor: [100, 134, 114], textColor: 255, fontStyle: 'bold' },
    footStyles: { fillColor: [236, 240, 238], textColor: 20, fontStyle: 'bold' },
  ...
  if (config.footerRow) {
    options.foot = [config.columns.map((column) => formatForDisplay(column, column.accessor(config.footerRow as T)))]
  }

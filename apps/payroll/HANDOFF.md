...
- Modalidad de comisión: cada versión se registra como `COMISIÓN FIJA` o `POR ESCALA`. La fija captura un único porcentaje aplicable desde cero y sin límite. La escala inicia con dos niveles vacíos; el usuario captura manualmente cada corte y porcentaje y agrega sólo los niveles necesarios. Cambiar de modalidad crea una nueva versión y no modifica cálculos históricos.

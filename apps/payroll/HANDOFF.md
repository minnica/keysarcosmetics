...
- Distribución POS: las ventas se agrupan por `employeeId`, `branchId` y periodo. El total de nómina de cada empleado —comisión más movimientos netos— se reparte proporcionalmente entre las sucursales donde generó ventas. Si no existen ventas en el periodo, se utiliza su centro de costo asignado. La suma de las columnas de sucursal debe ser idéntica al total de nómina de los empleados visibles.
- Filtros y descargas: búsqueda, puesto y sucursal actualizan tabla, métricas, impresión, PDF y Excel. La salida incluye título, periodo, alcance, origen POS, conciliación y sólo los registros seleccionados; la orientación es horizontal y las columnas usan anchos compactos para impresión.
...
- Periodo actual: una corrida en borrador se muestra como vista previa para comprobar empleados y montos, pero mantiene bloqueados impresión, PDF y Excel. Al cerrar o pagar la corrida, los tres formatos quedan habilitados y toman todos los registros del periodo, no sólo la página visible.
- Historial: el selector de periodo permanece visible y controla simultáneamente pantalla, impresión, PDF y Excel. Los periodos anteriores conservan su fecha, estado y folio; la comisión de kiosco continúa usando cierres mensuales y las demás nóminas usan los cortes configurados.

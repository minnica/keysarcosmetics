...
- Historial y reportes: `Reportes > Liquidaciones` filtra por fecha de baja, nombre/puesto y sucursal, conserva alta/baja/pago/estatus y permite 20, 40, 60 o todos. La operación filtra además pagos pendientes o pagados y conserva casos cerrados sin retirarlos. Impresión, PDF y Excel descargan exclusivamente el filtro visible. El dashboard por sucursal incorpora sólo pagos realizados según las sucursales elegidas y alerta si falta centro de costo.
- Estados posibles: no aplica, borrador, autorizado y pagado; finiquito o liquidación; caso pendiente, cerrado como ganado o cerrado como liquidado.
- Acciones y permisos esperados: `module.settlements` controla la operación y `module.settlement_reports` el reporte; el usuario máster entra siempre. Producción deberá registrar motivo de baja, fundamento, actor, versión de cada cálculo, aprobación con segunda clave, documentos y evento contable inmutable.

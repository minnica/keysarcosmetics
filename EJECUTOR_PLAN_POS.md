...
Una tarea automática puede añadir un helper raíz sólo si es específico del POS y usa `scripts/{verify,prepare,provision,audit,check,seed}-pos-*` con extensión `.sh`, `.mjs` o `.ts`; sus pruebas pueden insertar `.test` antes de `.mjs`/`.ts`. Se prefiere `backend/api/scripts/` cuando el consumidor es únicamente el API. La allowlist admite `apps/pos/.env.example` exclusivamente como plantilla pública sin credenciales reales. El propio ejecutor, sus pruebas, scripts generales, otros archivos de entorno, artefactos visuales, manifests y secretos permanecen protegidos.
